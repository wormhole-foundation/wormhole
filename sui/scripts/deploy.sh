...
WORMHOLE_PUBLISH_OUTPUT=$($(echo node /app/main.js sui deploy "$WORMHOLE_PATH" -n "$NETWORK" "$PRIVATE_KEY_ARG"))
...
WORMHOLE_INIT_OUTPUT=$($(echo node /app/main.js sui init-wormhole -n "$NETWORK" --initial-guardian "$GUARDIAN_ADDR" -p "$WORMHOLE_PACKAGE_ID" "$PRIVATE_KEY_ARG"))
...
TOKEN_BRIDGE_PUBLISH_OUTPUT=$($(echo node /app/main.js sui deploy "$TOKEN_BRIDGE_PATH" -n "$NETWORK" "$PRIVATE_KEY_ARG"))
...
TOKEN_BRIDGE_INIT_OUTPUT=$($(echo node /app/main.js sui init-token-bridge -n "$NETWORK" -p "$TOKEN_BRIDGE_PACKAGE_ID" -w "$WORMHOLE_STATE_OBJECT_ID" "$PRIVATE_KEY_ARG"))
...
if [ "$NETWORK" = devnet ]; then
  ...
  EXAMPLE_APP_PUBLISH_OUTPUT=$($(echo node /app/main.js sui deploy "$EXAMPLE_APP_PATH" -n "$NETWORK" "$PRIVATE_KEY_ARG"))
  EXAMPLE_APP_PACKAGE_ID=$(echo "$EXAMPLE_APP_PUBLISH_OUTPUT" | grep -oP 'Published to +\K.*')
  echo "$EXAMPLE_APP_PUBLISH_OUTPUT"

  EXAMPLE_INIT_OUTPUT=$($(echo node /app/main.js sui init-example-message-app -n "$NETWORK" -p "$EXAMPLE_APP_PACKAGE_ID" -w "$WORMHOLE_STATE_OBJECT_ID" "$PRIVATE_KEY_ARG"))
  ...
  EXAMPLE_COIN_PUBLISH_OUTPUT=$($(echo node /app/main.js sui deploy "$EXAMPLE_COIN_PATH" -n "$NETWORK" "$PRIVATE_KEY_ARG"))
  ...
  echo -e "\nPublish message command:" node /app/main.js sui publish-example-message -n devnet -p "$EXAMPLE_APP_PACKAGE_ID" -s "$EXAMPLE_APP_STATE_OBJECT_ID" -w "$WORMHOLE_STATE_OBJECT_ID" -m "hello" "$PRIVATE_KEY_ARG"
fi
