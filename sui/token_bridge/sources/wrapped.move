...
module token_bridge::wrapped_test {
    // try to register a wrapped asset as a native asset - should fail
    // #[expected_failure(abort_code = E_IS_WRAPPED_ASSET)] // E_IS_WRAPPED_ASSET
    // fun test_register_native_coin_fail(s: Scenario){
    //     let (admin, fee_recipient_person, _) = people();
    //     // register wrapped coin type COIN_WITNESS with the token bridge
    //     let test = set_up_wormhole_core_and_token_bridges(admin, s);
    ...
    //         );
    //     };
    //     // fail to register wrapped asset type with the token bridge using register_native_asset
    //     next_tx(&mut test, admin);{
    //         let bridge_state = take_shared<BridgeState>(&test);
    //         let worm_state = take_shared<State>(&test);
    //         let coin_meta = take_shared<CoinMetadata<COIN_WITNESS>>(&test);
    //         bridge_state::register_native_asset<COIN_WITNESS>(
    //             &mut worm_state,
    //             &mut bridge_state,
    //             &coin_meta,
    //             ctx(&mut test)
