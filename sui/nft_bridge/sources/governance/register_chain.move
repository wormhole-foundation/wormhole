...
module nft_bridge::register_chain {
    ...
    use nft_bridge::state::{Self, State, LatestOnly};
    ...
    public fun authorize_governance(
        nft_bridge_state: &State
    ): DecreeTicket<GovernanceWitness> {
        governance_message::authorize_verify_global(
            GovernanceWitness {},
            state::governance_chain(nft_bridge_state),
            state::governance_contract(nft_bridge_state),
    ...
    public fun register_chain(
        ...
        let latest_only = state::assert_latest_only(nft_bridge_state);
        ...
        handle_register_chain(&latest_only, nft_bridge_state, payload)
    }
    ...
    fun register_new_emitter(
        ...
        let registry =
            state::borrow_mut_emitter_registry(latest_only, nft_bridge_state);
        assert!(
...
#[test_only]
module nft_bridge::register_chain_tests {
    use sui::table::{Self};
    ...
    use nft_bridge::register_chain::{Self};
    use nft_bridge::state::{Self};
    use nft_bridge::nft_bridge_scenario::{
        person,
        return_state,
        set_up_wormhole_and_nft_bridge,
        take_state
    };
    ...
    fun test_register_chain() {
        // Testing this method.
        use nft_bridge::register_chain::{register_chain};
    ...
    fun test_cannot_register_chain_already_registered() {
        ...
        let expected_contract =
            *table::borrow(
                state::borrow_emitter_registry(&nft_bridge_state),
                chain
            );
