...
impl<'de> Deserializer<'de> {
    ...
    /// Should be called once the value has been fully deserialized, allowing the `Deserializer` to
    /// check if there is any trailing data in the input buffer. Returns an error if any trailing
    /// data is found.
...
mod tests {
    ...
    fn strings() {
        ...
        from_slice::<String>(buf.as_bytes()).expect_err("deserialized string");
        from_slice::<&str>(buf.as_bytes()).expect_err("deserialized string");
    }
