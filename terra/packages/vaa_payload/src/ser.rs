...
impl<'a, W: Write> ser::Serializer for &'a mut Serializer<W> {
    ...
    fn serialize_bytes(self, v: &[u8]) -> Result<Self::Ok, Self::Error> {
        let len = u8::try_from(v.len()).map_err(|_| Error::DataTooLarge(v.len()))?;
    ...
    fn serialize_seq(self, len: Option<usize>) -> Result<Self::SerializeSeq, Self::Error> {
        let len = len
            ...
            .and_then(|v| u8::try_from(v).map_err(|_| Error::DataTooLarge(v)))?;
...
mod tests {
    ...
    fn strings() {
        ...
        to_vec(buf).expect_err("serialized string");
        to_vec(&buf.to_string()).expect_err("serialized string");
    }
    ...
    fn data_too_large() {
        ...
        assert!(matches!(e, Error::DataTooLarge(300)));
    }
