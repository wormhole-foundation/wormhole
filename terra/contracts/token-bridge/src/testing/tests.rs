...
fn deserialize_transfer_vaa() -> StdResult<()> {
    ...
    assert_eq!(message.action, Action::TRANSFER, "message.action != expected");
    ...
    let token_address = hex::decode(token_address).unwrap();
    assert_eq!(info.token_address, token_address, "info.token_address != expected");

    let token_chain = 3u16;
    assert_eq!(info.token_chain, token_chain, "info.token_chain != expected");
    ...
    assert_eq!(info.recipient_chain, recipient_chain, "info.recipient_chain != expected");
...
fn deserialize_transfer_with_payload_vaa() -> StdResult<()> {
    ...
    let parsed = ParsedVAA::deserialize(signed_vaa.as_slice())?;
    let message = TokenBridgeMessage::deserialize(&parsed.payload)?;
    assert_eq!(message.action, Action::TRANSFER_WITH_PAYLOAD, "message.action != expected");
    ...
    assert_eq!(info_with_payload.payload.as_slice(), transfer_payload, "info.payload != expected");
