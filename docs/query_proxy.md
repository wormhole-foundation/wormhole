...
wormhole $build/bin/guardiand query-server \
      ...
      --ccqFactoryAddress "0xYourCCQFactoryAddressHere" \
...
- The `ccqFactoryAddress` specifies the address of the CCQ staking factory contract for staking-based rate limiting.
  If not provided, staking-based rate limiting is disabled and only basic DoS protection applies.
...
When staking-based rate limiting is enabled (`--ccqFactoryAddress` is provided):
...
3. The proxy fetches the staking policy from the factory contract (with caching)
...
If the factory address is not provided, only basic DoS protection rate limiting applies (10 req/sec per address,
burst of 20).
