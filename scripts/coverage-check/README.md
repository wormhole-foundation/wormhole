...
The baseline file (`.coverage-baseline` at repo root) lists packages and their minimum allowed coverage:
...
- name: Run golang tests with coverage (sdk)
  run: cd sdk && go test -v -timeout 5m -race -cover ./... 2>&1 | tee ../coverage-sdk.tmp
...
1. **Input**: Reads `coverage.txt` at repo root (generated by `go test -cover ./... | tee coverage.txt`)
2. **Parse**: Extracts package coverage percentages using regex
3. **Compare**: Checks against baseline file (`.coverage-baseline`)
...
**Key functions:**
- `parseCoverageOutput()` - Reads `coverage.txt` and extracts percentages
- `loadBaseline()` - Parses `.coverage-baseline`
