...
fn main() -> Result<(), ErrBox> {
    ...
    match cli.action {
        ...
        Action::Attest {
            ref attestation_cfg,
        } => handle_attest(&rpc_client, payer, p2w_addr, attestation_cfg.as_path())?,
    }
...
fn handle_attest(
    ...
    let batch_count = {
        let mut whole_batches = attestation_cfg.symbols.len() / config.max_batch_size as usize;

        // Include the partial batch if there is a remainder
