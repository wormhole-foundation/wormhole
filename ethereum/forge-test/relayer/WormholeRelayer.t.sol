...
import {DeliveryProviderSetup} from "../../contracts/relayer/deliveryProvider/DeliveryProviderSetup.sol";
import {DeliveryProviderImplementation} from "../../contracts/relayer/deliveryProvider/DeliveryProviderImplementation.sol";
import {DeliveryProviderProxy} from "../../contracts/relayer/deliveryProvider/DeliveryProviderProxy.sol";
import {DeliveryProviderStructs} from "../../contracts/relayer/deliveryProvider/DeliveryProviderStructs.sol";
import "../../contracts/interfaces/relayer/IWormholeRelayerTyped.sol";
import {DeliveryInstruction, RedeliveryInstruction, DeliveryOverride, EvmDeliveryInstruction} from "../../contracts/relayer/libraries/RelayerInternalStructs.sol";
...
import {EvmExecutionInfoV1, ExecutionInfoVersion, decodeEvmExecutionInfoV1, encodeEvmExecutionInfoV1} from "../../contracts/relayer/libraries/ExecutionParameters.sol";
...
contract WormholeRelayerTests is Test {
    ...
    function toGasParametersTyped(
        GasParameters memory gasParams
    ) internal pure returns (GasParametersTyped memory) {
        return
            GasParametersTyped({
    ...
    function toFeeParametersTyped(
        FeeParameters memory feeParams
    ) internal pure returns (FeeParametersTyped memory) {
        return
            FeeParametersTyped({
    ...
    function setUp() public {
        ...
        relayerWormholeSimulator = new FakeWormholeSimulator(wormhole);

        helpers = new TestHelpers();

        genericRelayer = new MockGenericRelayer(
            address(wormhole),
            address(relayerWormholeSimulator)
        );
    ...
    function standardAssume(
        ...
        vm.assume(
            (uint256(gasParams.sourceGasPrice) *
                feeParams.sourceNativePrice +
                feeParams.targetNativePrice -
                1) /
                uint256(feeParams.targetNativePrice) <
                type(uint88).max
        );
        vm.assume(
            (uint256(gasParams.targetGasPrice) *
                ...
                feeParams.sourceNativePrice -
                1) /
                uint256(feeParams.sourceNativePrice) <
                type(uint88).max
        );
        ...
        vm.assume(
            feeParams.targetNativePrice <
                (uint256(2) ** 126) /
                    (uint256(1) *
                        gasParams.targetGasPrice *
                        (uint256(0) +
                            gasParams.targetGasLimit +
                            gasParams.evmGasOverhead) +
        ...
        vm.assume(
            feeParams.sourceNativePrice <
                (uint256(2) ** 126) /
                    (uint256(1) *
                        gasParams.sourceGasPrice *
                        (uint256(gasParams.targetGasLimit) +
                            gasParams.evmGasOverhead) +
    ...
    function standardAssumeAndSetupTwoChains(
        ...
        standardAssume(
            gasParams_,
            feeParams_,
            uint32(minTargetGasLimit.unwrap())
        );
        ...
        s.source.deliveryProvider.updateDeliverGasOverhead(
            s.targetChain,
            gasParams.evmGasOverhead
        );
        s.target.deliveryProvider.updateDeliverGasOverhead(
            s.sourceChain,
            gasParams.evmGasOverhead
        );

        s.source.wormholeSimulator.setMessageFee(
            feeParams.wormholeFeeOnSource.unwrap()
        );
        s.target.wormholeSimulator.setMessageFee(
            feeParams.wormholeFeeOnTarget.unwrap()
        );
    }
    ...
    function setUpChains(uint16 numChains) internal {
        for (uint16 i = 1; i <= numChains; i++) {
            ...
            (mapEntry.wormhole, mapEntry.wormholeSimulator) = helpers
                .setUpWormhole(i);
            mapEntry.deliveryProvider = helpers.setUpDeliveryProvider(i);
            mapEntry.deliveryProvider.updateSupportedMessageKeyTypes(
                VAA_KEY_TYPE,
                true
            );
            mapEntry.coreRelayer = helpers.setUpWormholeRelayer(
                mapEntry.wormhole,
                address(mapEntry.deliveryProvider)
            );
            mapEntry.coreRelayerFull = WormholeRelayer(
                payable(address(mapEntry.coreRelayer))
            );
            genericRelayer.setWormholeRelayerContract(
                i,
                address(mapEntry.coreRelayer)
            );
            mapEntry.integration = new MockRelayerIntegration(
                address(mapEntry.wormhole),
                address(mapEntry.coreRelayer)
            );
            mapEntry.relayer = address(
                uint160(
                    uint256(keccak256(abi.encodePacked(bytes("relayer"), i)))
                )
            );
            ...
            mapEntry.refundAddress = payable(
                address(
                    uint160(
                        uint256(
                            keccak256(
                                abi.encodePacked(bytes("refundAddress"), i)
                            )
                        )
                    )
                )
            ...
            mapEntry.rewardAddress = payable(
                address(
                    uint160(
                        uint256(
                            keccak256(
                                abi.encodePacked(bytes("rewardAddress"), i)
                            )
                        )
                    )
                )
        ...
        for (uint16 i = 1; i <= numChains; i++) {
            for (uint16 j = 1; j <= numChains; j++) {
                ...
                map[i].deliveryProvider.updateAssetConversionBuffer(
                    j,
                    500,
                    10000
                );
                map[i].deliveryProvider.updateTargetChainAddress(
                    j,
                    bytes32(uint256(uint160(address(map[j].deliveryProvider))))
                );
                map[i].deliveryProvider.updateRewardAddress(
                    map[i].rewardAddress
                );
                ...
                map[i].deliveryProvider.updateMaximumBudget(
                    j,
                    Wei.wrap(maxBudget)
                );
                map[i].integration.registerEmitter(
                    j,
                    bytes32(uint256(uint160(address(map[j].integration))))
                );
                XAddress[] memory addresses = new XAddress[](1);
                addresses[0] = XAddress(
                    j,
                    bytes32(uint256(uint160(address(map[j].integration))))
                );
    ...
    function getDeliveryVAAHash(
        Vm.Log[] memory logs
    ) internal pure returns (bytes32 vaaHash) {
        (vaaHash, ) = logs[0].data.asBytes32(0);
    }

    function getDeliveryStatus(
        Vm.Log memory log
    ) internal view returns (IWormholeRelayerDelivery.DeliveryStatus status) {
    ...
    function getRefundStatus(
        Vm.Log memory log
    ) internal pure returns (IWormholeRelayerDelivery.RefundStatus status) {
    ...
    function resendMessageToTargetChain(
        ...
        (LocalNative newDeliveryCost, ) = setup
            .source
            .coreRelayer
            .quoteEVMDeliveryPrice(
                setup.targetChain,
    ...
    function testSend(
        ...
        assertTrue(
            getDeliveryStatus() ==
                IWormholeRelayerDelivery.DeliveryStatus.SUCCESS
        );
    }
    ...
    function testResend(
        ...
        uint64 sequence = sendMessageToTargetChain(
            setup,
            TOO_LOW_GAS_LIMIT,
            0,
            message
        ...
        resendMessageToTargetChain(
            setup,
            sequence,
            uint32(REASONABLE_GAS_LIMIT.unwrap()),
            0,
            message
        );
    ...
    function testRevertUnsupportedMessageKeyType(
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                DeliveryProviderDoesNotSupportMessageKeyType.selector,
                RANDOM_KEY_TYPE
            )
        );
    ...
    function testMultipleForwards(
        ...
        (LocalNative firstForwardDeliveryCost, ) = setup
            .target
            .coreRelayer
            .quoteEVMDeliveryPrice(
                setup.sourceChain,
        ...
        (LocalNative secondForwardDeliveryCost, ) = setup
            .target
            .coreRelayer
            .quoteEVMDeliveryPrice(
                setup.targetChain,
        ...
        uint256 receiverValue = firstForwardDeliveryCost.unwrap() +
            secondForwardDeliveryCost.unwrap();
    ...
    function testResendFailAndSucceed(
        ...
        for (uint32 i = 2; i < 10; i++) {
            resendMessageToTargetChain(
                setup,
                sequence,
                uint32(TOO_LOW_GAS_LIMIT.unwrap() * i),
                0,
                message
            );
            genericRelayer.relay(setup.sourceChain);
    ...
    {
        return
            setupFundsCorrectTest(
                gasParams_,
                feeParams_,
                uint32(minGasLimit.unwrap())
            );
    }
    ...
    function setupFundsCorrectTest(
        ...
        returns (StandardSetupTwoChains memory s, FundsCorrectTest memory test)
    {
        s = standardAssumeAndSetupTwoChains(
            gasParams_,
            feeParams_,
            Gas.wrap(minGasLimit)
        );
        ...
        test.targetChainRefundPerGasUnused = targetChainRefundPerGasUnused
            .unwrap();
    ...
    function testFundsCorrectForASend(
        ...
        test.refundAddressAmount =
            setup.target.refundAddress.balance -
            test.refundAddressBalance;
        test.rewardAddressAmount =
            setup.source.rewardAddress.balance -
            test.rewardAddressBalance;
        test.relayerPayment =
            test.relayerBalance -
            setup.target.relayer.balance;
        test.destinationAmount =
            address(setup.target.integration).balance -
            test.destinationBalance;
        ...
        assertTrue(
            test.relayerPayment ==
                test.destinationAmount + test.refundAddressAmount,
    ...
    function testFundsCorrectForASendFailureDueToGasExceeded(
        ...
        assertTrue(
            ...
            "No receiver value was sent to the contract"
        );
        assertTrue(
            test.rewardAddressAmount + feeParams.wormholeFeeOnSource ==
                test.deliveryPrice,
        ...
        assertTrue(
            test.refundAddressAmount == test.receiverValue,
    ...
    function testFundsCorrectForASendCrossChainRefundSuccess(
        ...
        uint256 amountToGetInRefundTarget = (setup
            .target
            .rewardAddress
            .balance - refundRewardAddressBalance);

        vm.assume(amountToGetInRefundTarget > 0);

        uint256 refundSource;
        (LocalNative baseFee, ) = setup
            .target
            .coreRelayer
            .quoteEVMDeliveryPrice(
                setup.sourceChain,
    ...
    function testFundsCorrectForASendCrossChainRefundFailProviderNotSupported(
        ...
        (
            ...
            FundsCorrectTest memory test
                ...
                gasParams,
                feeParams,
                uint32(170000 + REASONABLE_GAS_LIMIT.unwrap())
            );

        setup.target.deliveryProvider.updateSupportedChain(
            setup.sourceChain,
            false
        );
        vm.assume(
            test.targetChainRefundPerGasUnused *
                REASONABLE_GAS_LIMIT.unwrap() >=
                feeParams.wormholeFeeOnTarget +
                    uint256(1) *
                    gasParams.evmGasOverhead *
                    gasParams.sourceGasPrice *
    ...
    bytes constant RANDOM_KEY_TYPE_BODY =
        hex"12911894719274912740817248912740817240";
    ...
    function testUnitTestSend(
        ...
        for (uint256 j = 0; j < 3; j++) {
            messageKeys[j] = MessageKey(
                VAA_KEY_TYPE,
                WormholeRelayerSerde.encodeVaaKey(params.vaaKeysFixed[j])
            );
        }
        setup.source.deliveryProvider.updateSupportedMessageKeyTypes(
            RANDOM_KEY_TYPE,
            true
        );
        ...
        ) = setup.source.coreRelayer.quoteEVMDeliveryPrice(
                setup.targetChain,
                TargetNative.wrap(params.receiverValue),
                Gas.wrap(params.gasLimit)
            );
        ...
        TargetNative extraReceiverValue = setup
            .source
            .coreRelayer
            .quoteNativeForChain(
                setup.targetChain,
        ...
        DeliveryInstruction memory expectedInstruction = DeliveryInstruction({
            ...
            refundDeliveryProvider: setup
                .source
                .deliveryProvider
                .getTargetChainAddress(setup.targetChain),
            sourceDeliveryProvider: toWormholeFormat(
                address(setup.source.deliveryProvider)
            ),
        ...
        checkInstructionEquality(
            relayerWormholeSimulator
                .parseVMFromLogs(vm.getRecordedLogs()[0])
                .payload,
            expectedInstruction
        );
    }
    ...
    function testUnitTestResend(
        ...
        RedeliveryInstruction
            memory expectedInstruction = RedeliveryInstruction({
                deliveryVaaKey: params.deliveryVaaKey,
                targetChain: setup.targetChain,
                newRequestedReceiverValue: TargetNative.wrap(
                    params.newReceiverValue
                ),
                newEncodedExecutionInfo: encodedExecutionInfo,
                newSourceDeliveryProvider: toWormholeFormat(
                    address(setup.source.deliveryProvider)
                ),
    ...
    function checkMessageKey(
        ...
        if (decodedMessageKey.keyType == VAA_KEY_TYPE) {
            ...
            assertEq(
                encodedKey,
                messageKey.encodedKey,
                "decoded encodedKey must equal expected"
            );
    ...
    function checkVaaKey(
        ...
        console.log(data.length, index);
        assertTrue(
            decodedVaaKey.emitterAddress == vaaKey.emitterAddress,
    ...
    function checkInstructionEquality(
        ...
        assertTrue(
            decodedInstruction.targetAddress ==
                expectedInstruction.targetAddress,
            "Wrong target address"
        );
        (length, index) = data.asUint32(index);
        (decodedInstruction.payload, index) = data.slice(index, length);
        assertTrue(
            keccak256(decodedInstruction.payload) ==
                keccak256(expectedInstruction.payload),
            "Wrong payload"
        );
        ...
        assertTrue(
            requestedReceiverValue ==
                expectedInstruction.requestedReceiverValue.unwrap(),
            "Wrong requested receiver value"
        );
        (extraReceiverValue, index) = data.asUint256(index);
        assertTrue(
            extraReceiverValue ==
                expectedInstruction.extraReceiverValue.unwrap(),
            "Wrong extra receiver value"
        );
        (length, index) = data.asUint32(index);
        (decodedInstruction.encodedExecutionInfo, index) = data.slice(
            index,
            length
        );
        assertTrue(
        ...
        assertTrue(
            decodedInstruction.refundAddress ==
                expectedInstruction.refundAddress,
            "Wrong refund address"
        );
        (decodedInstruction.refundDeliveryProvider, index) = data.asBytes32(
            index
        );
        assertTrue(
            decodedInstruction.refundDeliveryProvider ==
                expectedInstruction.refundDeliveryProvider,
            "Wrong refund relay provider"
        );
        (decodedInstruction.sourceDeliveryProvider, index) = data.asBytes32(
            index
        );
        assertTrue(
            decodedInstruction.sourceDeliveryProvider ==
                expectedInstruction.sourceDeliveryProvider,
            "Wrong source relay provider"
        );
        (decodedInstruction.senderAddress, index) = data.asBytes32(index);
        assertTrue(
            decodedInstruction.senderAddress ==
                expectedInstruction.senderAddress,
        ...
        for (uint256 i = 0; i < messageKeysLength; i++) {
            index = checkMessageKey(
                data,
                index,
                expectedInstruction.messageKeys[i]
            );
        }
    ...
    function checkRedeliveryInstructionEquality(
        ...
        assertTrue(
            requestedReceiverValue ==
                expectedInstruction.newRequestedReceiverValue.unwrap(),
            "Wrong requested receiver value"
        );
        (length, index) = data.asUint32(index);
        (decodedInstruction.newEncodedExecutionInfo, index) = data.slice(
            index,
            length
        );
        assertTrue(
        ...
        (decodedInstruction.newSourceDeliveryProvider, index) = data.asBytes32(
            index
        );
        assertTrue(
        ...
        assertTrue(
            decodedInstruction.newSenderAddress ==
                expectedInstruction.newSenderAddress,
            "Wrong sender address"
        );
        assertTrue(index == data.length, "Wrong length of data");
    }
    ...
    function prepareDeliveryStack(
        ...
        stack.encodedDeliveryVAA = relayerWormholeSimulator
            .fetchSignedMessageFromLogs(
                stack.entries[numVaas],
                setup.sourceChain,
                address(setup.source.coreRelayer)
            );

        stack.relayerRefundAddress = payable(setup.target.relayer);
        stack.parsed = relayerWormhole.parseVM(stack.encodedDeliveryVAA);
        stack.instruction = WormholeRelayerSerde.decodeDeliveryInstruction(
    ...
    function testRevertDeliveryInvalidDeliveryVAA(
        ...
        vm.expectRevert(
            abi.encodeWithSignature("InvalidDeliveryVaa(string)", "")
        );
    ...
    function testRevertDeliveryInvalidEmitter(
        ...
        IWormhole.VM memory vm_ = relayerWormholeSimulator.parseVMFromLogs(
            stack.entries[0]
        );
        vm_.version = uint8(1);
        vm_.timestamp = uint32(block.timestamp);
        vm_.emitterChainId = setup.sourceChain;
        vm_.emitterAddress = toWormholeFormat(
            address(setup.source.integration)
        );
        bytes memory deliveryVaaWithWrongEmitter = relayerWormholeSimulator
            .encodeAndSignMessage(vm_);
    ...
    function testRevertDeliveryTargetChainIsNotThisChain(
        ...
        vm.prank(setup.target.relayer);
        vm.expectRevert(
            abi.encodeWithSignature("TargetChainIsNotThisChain(uint16)", 2)
        );
        map[setup.differentChainId].coreRelayerFull.deliver{
            value: stack.budget
        }(
            stack.encodedVMs,
            stack.encodedDeliveryVAA,
    ...
    function testNoFalseFiresReplayProtection(
        ...
        assertEq(
            setup.target.coreRelayerFull.deliverySuccessBlock(
                stack.deliveryVaaHash
            ),
            block.number
        );
        assertEq(
            setup.target.coreRelayerFull.deliveryFailureBlock(
                stack.deliveryVaaHash
            ),
            0
        );
    ...
    function testReplayProtection(
        ...
        vm.prank(setup.target.relayer);
        assertFalse(
            setup.target.coreRelayerFull.deliveryAttempted(
                stack.deliveryVaaHash
            )
        );
        setup.target.coreRelayerFull.deliver{value: stack.budget}(
    ...
    function testRevertDeliveryVaaKeysLengthDoesNotMatchVaasLength(
        ...
        vm.expectRevert(
            abi.encodeWithSignature(
                "MessageKeysLengthDoesNotMatchMessagesLength(uint256,uint256)",
                0,
                1
            )
        );
    ...
    function testRevertDeliveryVaaKeysDoNotMatchVaas(
        ...
        uint64 sequence = setup.source.wormhole.publishMessage{
            value: feeParams.wormholeFeeOnSource
        }(1, bytes(""), 200);
        ...
        vm.prank(setup.target.relayer);
        vm.expectRevert(
            abi.encodeWithSignature("VaaKeysDoNotMatchVaas(uint8)", 0)
        );
        setup.target.coreRelayerFull.deliver{value: stack.budget}(
            stack.encodedVMs,
            stack.encodedDeliveryVAA,
    ...
    function testRevertDeliveryWithOverrideReceiverValue(
        ...
        vm.expectRevert(
            abi.encodeWithSignature("InvalidOverrideReceiverValue()")
        );
        setup.target.coreRelayerFull.deliver{value: stack.budget}(
            stack.encodedVMs,
            stack.encodedDeliveryVAA,
    ...
    function testRevertDeliveryWithOverrideMaximumRefund(
        ...
        vm.expectRevert(
            abi.encodeWithSignature("InvalidOverrideRefundPerGasUnused()")
        );
        setup.target.coreRelayerFull.deliver{value: stack.budget}(
            stack.encodedVMs,
            stack.encodedDeliveryVAA,
    ...
    function testSendCheckConsistencyLevel(
        ...
        (uint8 actualConsistencyLevel, ) = log.data.asUint8(
            32 + 32 + 32 + 32 - 1
        );
    ...
    function testRevertDeliveryReentrantCall(
        ...
        setup.source.integration.sendMessageWithReentrantDelivery{
            value: deliveryCost.unwrap()
    ...
    function testEncodeAndDecodeVaaKey() public {
        ...
        (VaaKey memory newVaaKey, ) = WormholeRelayerSerde.decodeVaaKey(
            WormholeRelayerSerde.encodeVaaKey(vaaKey),
            0
        );
    ...
    function testEncodeAndDecodeMessageKey() public {
        ...
        (MessageKey memory newMessageKey, ) = WormholeRelayerSerde
            .decodeMessageKey(
                WormholeRelayerSerde.encodeMessageKey(messageKey),
                0
            );
        checkMessageKey(
            WormholeRelayerSerde.encodeMessageKey(newMessageKey),
            0,
            messageKey
        );
    }
    ...
    function testRevertEncodeAndDecodeTooLongMessageKeyArray() public {
        ...
        vm.expectRevert(
            abi.encodeWithSignature("TooManyMessageKeys(uint256)", len)
        );
    ...
    function testEncodeAndDecodeMessageKeyArray(uint8 len, uint8 idx) public {
        ...
        (MessageKey[] memory newMessageKeys, ) = WormholeRelayerSerde
            .decodeMessageKeyArray(
                WormholeRelayerSerde.encodeMessageKeyArray(messageKeys),
                0
            );
        for (uint256 i = 0; i < len; ++i) {
            checkMessageKey(
                WormholeRelayerSerde.encodeMessageKey(newMessageKeys[i]),
                0,
                messageKeys[i]
            );
        }
    ...
    function testEncodeAndDecodeDeliveryInstruction(
        ...
        DeliveryInstruction memory newInstruction = WormholeRelayerSerde
            .decodeDeliveryInstruction(
                WormholeRelayerSerde.encode(instruction)
            );
        ...
        checkInstructionEquality(
            WormholeRelayerSerde.encode(newInstruction),
            instruction
        );
    }
    ...
    function testDeliveryData(
        ...
        DeliveryData memory deliveryData = setup
            .target
            .integration
            .getDeliveryData();
        ...
        assertTrue(
            deliveryData.sourceChain == setup.sourceChain,
            "Source chain id wrong"
        );
        assertTrue(
            deliveryData.deliveryHash == deliveryVaaHash,
            "delivery vaa hash wrong"
        );
        assertTrue(
            keccak256(setup.target.integration.getMessage()) ==
                keccak256(message),
