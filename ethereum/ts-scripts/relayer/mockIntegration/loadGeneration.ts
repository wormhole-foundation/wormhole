...
async function run() {
  const chainIntervalIdx = process.argv.findIndex(
    (arg) => arg === "--chainInterval"
  );
  const salvoIntervalIdx = process.argv.findIndex(
    (arg) => arg === "--salvoInterval"
  );
  const chainInterval =
    chainIntervalIdx !== -1
      ? Number(process.argv[chainIntervalIdx + 1])
      : 5_000;
  const salvoInterval =
    salvoIntervalIdx !== -1
      ? Number(process.argv[salvoIntervalIdx + 1])
      : 60_000;
...
async function matrix(chainIntervalMS: number, salvoIntervalMS: number) {
  console.log(
    `Sending test messages to and from every combination of chains...`
  );
  for (let salvo = 0; true; salvo++) {
    console.log("");
    console.log(`Sending salvo ${salvo}`);
