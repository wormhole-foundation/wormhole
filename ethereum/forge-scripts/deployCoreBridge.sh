...
FOUNDRY_PROFILE=production forge script ./forge-scripts/DeployCore.s.sol:DeployCore --sig "run(address[],uint16,uint16,bytes32,uint256)" $INIT_SIGNERS $INIT_CHAIN_ID $INIT_GOV_CHAIN_ID $INIT_GOV_CONTRACT $INIT_EVM_CHAIN_ID --rpc-url "$RPC_URL" --private-key "$PRIVATE_KEY" --broadcast 
