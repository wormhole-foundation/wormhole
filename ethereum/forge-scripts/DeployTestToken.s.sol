...
contract DeployTestToken is Script {
    ...
    function _deploy() internal returns (address deployedTokenAddress, address deployedNFTaddress, address deployedWETHaddress, address deployedAccountantTokenAddress) {
        
        ERC20PresetMinterPauser token = new ERC20PresetMinterPauser("Ethereum Test Token", "TKN");
        console.log("Token deployed at: ", address(token));

        // mint 1000 units
        token.mint(address(this), 1_000_000_000_000_000_000_000);

        ERC721PresetMinterPauserAutoId nft = new ERC721PresetMinterPauserAutoId(unicode"Not an APE🐒", unicode"APE🐒", "https://cloudflare-ipfs.com/ipfs/QmeSjSinHpPnmXmspMjwiXyN6zS4E9zccariGR3jxcaWtq/");
        ...
        ERC20PresetMinterPauser accountantToken = new ERC20PresetMinterPauser("Accountant Test Token", "GA");

        console.log("Accountant test token deployed at: ", address(accountantToken));

        // mint 1000 units
        accountantToken.mint(address(this), 1_000_000_000_000_000_000_000);

        return (address(token), address(nft), address(mockWeth), address(accountantToken));
