...
contract NFTBridge is NFTBridgeGovernance {
    ...
    function transferNFT(
        ...
        if (isWrappedAsset(token)) {
            ...
            require(
                ERC165(token).supportsInterface(type(IERC721).interfaceId),
            ...
            require(
                ...
                "must support the ERC721-Metadata extension"
            );
        }
        ...
        string memory uriString;
        {
            ...
            (, bytes memory queriedURI) = token.staticcall(
    ...
    function logTransfer(
        NFTBridgeStructs.Transfer memory transfer,
        uint256 callValue,
        uint32 nonce
    ...
    function _completeTransfer(bytes memory encodedVm) internal {
        ...
        if (transfer.tokenChain != chainId()) {
            ...
            // mint wrapped asset
            NFTImplementation(address(transferToken)).mint(
                transferRecipient,
                transfer.tokenID,
                transfer.uri
            );
    ...
    function encodeTransfer(NFTBridgeStructs.Transfer memory transfer)
        public
        pure
    ...
    function parseTransfer(bytes memory encoded)
        public
        pure
        returns (NFTBridgeStructs.Transfer memory transfer)
    {
        uint256 index = 0;
