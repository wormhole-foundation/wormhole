...
function getDefaultRelayProviderState() pure returns (DefaultRelayProviderState storage state) {
  assembly ("memory-safe") { state.slot := DEFAULT_RELAY_PROVIDER_STORAGE_SLOT }
}

struct RegisteredCoreRelayersState {
  // chainId => wormhole address mapping of relayer contracts on other chains
  mapping(uint16 => bytes32) registeredCoreRelayers;
}
...
function getRegisteredCoreRelayersState() pure returns (RegisteredCoreRelayersState storage state) {
  assembly ("memory-safe") { state.slot := REGISTERED_CORE_RELAYERS_STORAGE_SLOT }
}
...
function getDeliveryTmpState() pure returns (DeliveryTmpState storage state) {
  assembly ("memory-safe") { state.slot := DELIVERY_TMP_STORAGE_SLOT }
}
