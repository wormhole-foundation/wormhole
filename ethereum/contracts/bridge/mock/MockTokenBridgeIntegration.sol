...
contract MockTokenBridgeIntegration {
    ...
    function completeTransferAndSwap(bytes memory encodedVm) public {
        ...
        address wrappedAddress = tokenBridge().wrappedAsset(
            tokenChainId,
            tokenAddress
        );
        IERC20 transferToken = IERC20(wrappedAddress);
        uint256 balanceBefore = transferToken.balanceOf(address(this));
        bytes memory payload = tokenBridge().completeTransferWithPayload(
            encodedVm,
            msg.sender
        );
        ...
        require(
            tokenAddress == vmTokenAddress,
            "Address parsed from VAA and payload do not match"
        );
        uint16 vmTokenChainId = payload.toUint16(65);
        require(
            tokenChainId == vmTokenChainId,
            "ChainId parsed from VAA and payload do not match"
        );
