...
import "./HederaTokenState.sol";
import "./IHederaTokenService.sol";
import "./HederaTokenService.sol";
import "./HederaResponseCodes.sol";
// had to provide path for openzeppelin. couldn't find node_module otherwise
...
contract HederaTokenImplementation is ExpiryHelper, HederaTokenService, HederaTokenState, Context {
    event Transfer(address indexed from, address indexed to, uint256 value);
    event Approval(address indexed owner, address indexed spender, uint256 value);
    /* needs to be payable in order to use the HTS & pay future rent */
    function initialize(
        string memory name_,
        ...
        // ** have not been able to run it with the below **
    ...
    function _initializeNativeToken(
        ...
        uint256 decimals__ = decimals_;
        (int responseCode, address tokenAddress) = HederaTokenService.createFungibleToken(token, initialTotalSupply, decimals__);
        if (int(responseCode) != int(HederaResponseCodes.SUCCESS)){
            revert("Failed to create fungible token");
        } /* there's an issue with dtypes. 
            it will revert if responseCode && HederaResponseCodes.SUCCESS are not the same dtypes */

        _state.hederaEVMAddress = tokenAddress;
    ...
    function name() public returns (string memory) {
        (int responseCode, IHederaTokenService.FungibleTokenInfo memory fungibleTokenInfo) = HederaTokenService.getFungibleTokenInfo(_state.hederaEVMAddress);
        if (int(responseCode) != int(HederaResponseCodes.SUCCESS)){
            revert("Failed to get name()");
        }
        return fungibleTokenInfo.tokenInfo.token.name;
    }

    function symbol() public returns (string memory) {
        (int responseCode, IHederaTokenService.FungibleTokenInfo memory fungibleTokenInfo) = HederaTokenService.getFungibleTokenInfo(_state.hederaEVMAddress);
        if (int(responseCode) != int(HederaResponseCodes.SUCCESS)){
            revert("Failed to get symbol()");
        }
        return fungibleTokenInfo.tokenInfo.token.name;
    }
    ...
    function hederaEVMAddress() public view returns (address) {
        return _state.hederaEVMAddress;
    }

    function decimals() public returns (uint32) {
        (int responseCode, IHederaTokenService.FungibleTokenInfo memory fungibleTokenInfo) = HederaTokenService.getFungibleTokenInfo(_state.hederaEVMAddress);
        if (int(responseCode) != int(HederaResponseCodes.SUCCESS)){
            revert("Failed to get decimals()");
        }
        return fungibleTokenInfo.decimals;
    }

    function totalSupply() public returns (uint256) {
        (int responseCode, IHederaTokenService.FungibleTokenInfo memory fungibleTokenInfo) = HederaTokenService.getFungibleTokenInfo(_state.hederaEVMAddress);
        if (int(responseCode) != int(HederaResponseCodes.SUCCESS)){
            revert("Failed to get totalSupply()");
        }
        uint256 totalSupply = fungibleTokenInfo.tokenInfo.totalSupply;
        return totalSupply;
    }
...
/* Have not tested updateDetails function */
...
//         _state.metaLastUpdatedSequence = sequence_;
//         (int32 responseCode, IHederaTokenService.FungibleTokenInfo memory fungibleTokenInfo) = HederaTokenService.getFungibleTokenInfo(_state.hederaEVMAddress);
//         if(responseCode != HederaResponseCodes.SUCCESS){
//             revert("Failed to get hedera token info");
//         }
//         IHederaTokenService.HederaToken memory token = fungibleTokenInfo.tokenInfo.token;
//         token.name = name_;
//         token.symbol = symbol_;
//         int32 responseCode2 = HederaTokenService.updateTokenInfo(_state.hederaEVMAddress, token);
//         if(responseCode2 != HederaResponseCodes.SUCCESS){
//             revert("Failed to update hedera token info");
//         }    
...
//     }
    ...
    // !!!! removed view here to compile. need to address
    function eip712Domain() public returns (
        bytes1 domainFields,
    ...
    // !!!! removed view here to compile. need to address
    function _eip712DomainNameHashed() internal returns (bytes32) {
