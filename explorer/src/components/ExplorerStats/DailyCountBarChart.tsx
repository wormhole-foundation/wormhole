...
import { chainColors, chainIdColors, makeDate, makeGroupName } from "../../utils/explorer"
...
const DailyCountBarChart = (props: DailyCountProps) => {
    ...
    return (
        <div style={{ height: 400, minWidth: 400, flex: '1', marginBottom: 40 }}>
            <Typography variant="h4" style={{ marginLeft: 20 }}>messages/day</Typography>
            <ResponsiveBar
            ...
            {props.showBottomLedgend && <div
                ...
                {chainEnums.slice(1).map((chain, index) => <div key={chain} style={{ display: 'flex', alignItems: 'center', }}>
                    <div style={{ background: chainColors[String(index + 1)], height: 16, width: 16, display: 'inline-block' }} />&nbsp;
                    <span>{chain}</span>
                </div>)}
            </div>}
        </div>
