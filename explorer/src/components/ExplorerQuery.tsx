import { CHAIN_ID_TERRA, getEmitterAddressSolana, nativeToHexString } from "@certusone/wormhole-sdk";
import { fromHex } from "@cosmjs/encoding";
import { CircularProgress, Typography } from '@mui/material';
import { PublicKey } from '@solana/web3.js';
import { arrayify, hexlify, isHexString, zeroPad } from "ethers/lib/utils";
        ...
        if (emitterChain && emitterAddress && sequence) {
            if (emitterChain === chainIDs["solana"]) {
                ...
                if (emitterAddress.startsWith('terra')) {
                    try {
                        paddedAddress = nativeToHexString(emitterAddress, CHAIN_ID_TERRA) || ""
                    } catch (_) {
                        // do nothing
                    }
                } else {
    ...
    return (
        <>
            {loading ? <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                ...
                error ? <Typography variant="h4">{error === "explorer.notFound" ? "Nothing found for that query. Please check your data and try again." : "Something went wrong. Please check your data and try again."}</Typography> :
                    message ? (
                        // <ExplorerSummary
                        //     {...props}
                        //     message={message}
