...
const ExplorerMessageSearchForm: React.FC<ExplorerMessageSearchProps> = ({ location, toggleFormType, formName }) => {
    const [emitterChain, setEmitterChain] = useState<ExplorerMessageSearchValues["emitterChain"]>()
    const [emitterAddress, setEmitterAddress] = useState<ExplorerMessageSearchValues["emitterAddress"]>()
    const [sequence, setSequence] = useState<ExplorerMessageSearchValues["sequence"]>()
    ...
    const handleSubmit = (event: React.ChangeEvent<HTMLFormElement>) => {
        ...
        if (emitterChain && emitterAddress && sequence) {
            navigate(`?emitterChain=${emitterChain}&emitterAddress=${emitterAddress}&sequence=${sequence}`)
        }
    };
    ...
    return (
        <>
            ...
            >
                ...
                <ExplorerFormSelect currentlyActive={formName} toggleFormType={toggleFormType} />
                ...
                <Autocomplete
                    ...
                    renderInput={(params: any) => <TextField {...params} label="emitter contract" />}
                    ...
                    options={(Object.entries(activeNetwork.chains[chainEnums[emitterChain || 1]?.toLowerCase()] || {}))
                        .filter(([key]) => key.includes(" "))
                        .map(([key, val]) => ({ label: `${key} (${truncateAddress(val)})`, value: val }))}
                />
