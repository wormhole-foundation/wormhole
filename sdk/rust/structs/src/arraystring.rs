...
mod test {
    ...
    fn end_to_end() {
        ...
        for i in 0..=32 {
            ...
            let buf = vaa_payload::to_vec(&expected).unwrap();

            assert_eq!(&zeroes[..i], &buf[..i]);
            assert_eq!(v.as_bytes()[i..], buf[i..]);

            let actual = vaa_payload::from_slice(&buf).unwrap();
    ...
    fn value_too_large() {
        ...
        vaa_payload::to_vec(&v).expect_err("successfully serialized string longer than 32 bytes");
    }
    ...
    fn buffer_too_small() {
        ...
        vaa_payload::from_slice::<MyString>(&b)
