...
describe("Integration Tests", () => {
  describe("Algorand tests", () => {
    ...
    test("Test parseVAA", (done) => {
      (async () => {
        try {
          ...
          const hexGUVAA: Uint8Array = hexStringToUint8Array(gUpgradeVAA);
          ...
          const hexRCVAA: Uint8Array = hexStringToUint8Array(regChainVAA);
          ...
          const hexCWVAA: Uint8Array = hexStringToUint8Array(createWrapVAA);
          ...
          const hexRedVAA: Uint8Array = hexStringToUint8Array(redeemVAA);
          ...
          const hexTxVAA: Uint8Array = hexStringToUint8Array(transferVAA);
    ...
    test("Algorand transfer native ALGO to Eth and back again", (done) => {
      (async () => {
        try {
          ...
          const startingAlgoBal = algoWalletBals.get(AlgoIndex);
          if (!startingAlgoBal) {
          ...
          const finalAlgoBal = algoWalletBals.get(AlgoIndex);
          if (!finalAlgoBal) {
          ...
          const secondFinalAlgoBal = algoWalletBals.get(AlgoIndex);
    ...
    test("Algorand create NORIUM, transfer to Eth and back again", (done) => {
      (async () => {
        try {
          console.log("Starting attestation...");
          const CORE_ID: number = 4;
    ...
    test("Transfer wrapped Luna from Terra to Algorand and back again", (done) => {
      (async () => {
        try {
          ...
          const tempBal = tbBals.get(assetIdCreated);
          if (tempBal) {
          ...
          const tmpBal = bals.get(assetIdCreated);
          if (tmpBal) {
          ...
          const tBal = retBals.get(assetIdCreated);
          if (tBal) {
