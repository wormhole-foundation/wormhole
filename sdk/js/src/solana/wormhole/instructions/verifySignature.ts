...
import { getGuardianSet, guardianSetKey } from "../accounts";
...
function createVerifySignaturesInstruction(
  ...
  const parsed = isBytes(vaa) ? parseVaa(vaa) : vaa;
  const methods = createReadOnlyWormholeProgramInterface(wormholeProgramId).methods.verifySignatures(
    signatureStatus as any
  );

  // @ts-ignore
  return methods._ixFn(...methods._args, {
    accounts: getVerifySignatureAccounts(wormholeProgramId, payer, signatureSet, parsed) as any,
    signers: undefined,
