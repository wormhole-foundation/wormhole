...
import { bridgeInfoKey, guardianSetKey, postedVaaKey } from "../accounts";
...
export function createPostVaaInstruction(
  ...
  const methods = createReadOnlyWormholeProgramInterface(wormholeProgramId).methods.postVaa(
    parsed.version,
    parsed.guardianSetIndex,
    parsed.timestamp,
    parsed.nonce,
    parsed.emitterChain,
    Buffer.from(parsed.emitterAddress, "hex") as any,
  ...
  return methods._ixFn(...methods._args, {
    accounts: getPostVaaAccounts(wormholeProgramId, payer, signatureSet, parsed) as any,
    signers: undefined,
...
export function getPostVaaAccounts(
  ...
  return {
    guardianSet: guardianSetKey(wormholeProgramId, parsed.guardianSetIndex),
    bridgeInfo: bridgeInfoKey(wormholeProgramId),
    signatureSet: new PublicKey(signatureSet),
    message: postedVaaKey(wormholeProgramId, parsed.hash),
