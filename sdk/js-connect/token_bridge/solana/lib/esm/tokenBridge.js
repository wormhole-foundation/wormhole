import { toChainId, toChainName, UniversalAddress, toNative, ErrNotWrapped, } from '@wormhole-foundation/connect-sdk';
import { SolanaUnsignedTransaction, SolanaPlatform, SolanaAddress, } from '@wormhole-foundation/connect-sdk-solana';
import { utils } from '@wormhole-foundation/wormhole-connect-sdk-core-solana';
import { Keypair, PublicKey, SystemProgram, Transaction, } from '@solana/web3.js';
import { ACCOUNT_SIZE, NATIVE_MINT, TOKEN_PROGRAM_ID, createAssociatedTokenAccountInstruction, createCloseAccountInstruction, createInitializeAccountInstruction, getAssociatedTokenAddress, getMinimumBalanceForRentExemptAccount, } from '@solana/spl-token';
...
export class SolanaTokenBridge {
    ...
    async *createAttestation(token, payer) {
        ...
        const transferIx = await utils.createBridgeFeeTransferInstruction(this.connection, this.coreAddress, senderAddress);
        const messageKey = Keypair.generate();
        const attestIx = createAttestTokenInstruction(this.connection, this.tokenBridge.programId, this.coreAddress, senderAddress, new SolanaAddress(token).toUint8Array(), messageKey.publicKey, nonce);
    ...
    async *submitAttestation(vaa, payer) {
        ...
        const transaction = new Transaction().add(createCreateWrappedInstruction(this.connection, this.tokenBridge.programId, this.coreAddress, senderAddress, vaa));
        transaction.recentBlockhash = blockhash;
        transaction.feePayer = senderAddress;
    ...
    async transferSol(sender, recipient, amount, payload) {
        ...
        const tokenBridgeTransferIx = payload
            ? createTransferNativeWithPayloadInstruction(this.connection, this.tokenBridge.programId, this.coreAddress, senderAddress, message.publicKey, ancillaryKeypair.publicKey, NATIVE_MINT, nonce, amount, recipientAddress, recipientChainId, payload)
            : createTransferNativeInstruction(this.connection, this.tokenBridge.programId, this.coreAddress, senderAddress, message.publicKey, ancillaryKeypair.publicKey, NATIVE_MINT, nonce, amount, relayerFee, recipientAddress, recipientChainId);
    ...
    async *transfer(sender, recipient, token, amount, payload) {
        ...
        if (isSolanaNative) {
            tokenBridgeTransferIx = payload
                ? createTransferNativeWithPayloadInstruction(this.connection, this.tokenBridge.programId, this.coreAddress, senderAddress, message.publicKey, senderTokenAddress, tokenAddress, nonce, amount, recipientAddress, recipientChainId, payload)
                : createTransferNativeInstruction(this.connection, this.tokenBridge.programId, this.coreAddress, senderAddress, message.publicKey, senderTokenAddress, tokenAddress, nonce, amount, relayerFee, recipientAddress, recipientChainId);
        }
        else {
            const originAsset = await this.getOriginalAsset(token);
            tokenBridgeTransferIx = payload
                ? createTransferWrappedWithPayloadInstruction(this.connection, this.tokenBridge.programId, this.coreAddress, senderAddress, message.publicKey, senderTokenAddress, senderAddress, toChainId(originAsset.chain), originAsset.address.toUint8Array(), nonce, amount, recipientAddress, recipientChainId, payload)
                : createTransferWrappedInstruction(this.connection, this.tokenBridge.programId, this.coreAddress, senderAddress, message.publicKey, senderTokenAddress, senderAddress, toChainId(originAsset.chain), originAsset.address.toUint8Array(), nonce, amount, relayerFee, recipientAddress, recipientChainId);
        }
    ...
    async *redeem(sender, vaa, unwrapNative = true) {
        ...
        if (acctInfo === null) {
            const ataCreationTx = new Transaction().add(createAssociatedTokenAccountInstruction(senderAddress, ataAddress, senderAddress, new PublicKey(wrappedToken.toUint8Array())));
        ...
        const transaction = new Transaction().add(createCompleteTransferInstruction(this.connection, this.tokenBridge.programId, this.coreAddress, senderAddress, vaa));
        transaction.recentBlockhash = blockhash;
        transaction.feePayer = senderAddress;
    ...
    async *postVaa(sender, vaa, blockhash) {
        ...
        const verifySignaturesInstructions = await utils.createVerifySignaturesInstructions(this.connection, this.coreAddress, senderAddr, vaa, signatureSet.publicKey);
        ...
        const postVaaTx = new Transaction().add(utils.createPostVaaInstruction(this.connection, this.coreAddress, senderAddr, vaa, signatureSet.publicKey));
    ...
    createUnsignedTx(txReq, description, parallelizable = false) {
        return new SolanaUnsignedTransaction(txReq, this.network, 'Solana', description, parallelizable);
    }
}
