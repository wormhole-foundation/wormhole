...
import {
  ...
  TEST_ERC20,
  WORMHOLE_RPC_HOSTS,
} from "../../../js/src/token_bridge/__tests__/utils/consts";
...
async function getEthTokenBridge(provider: ethers.Provider): Promise<TokenBridge<'Evm'>> {
  return EvmTokenBridge.fromProvider(provider, {
    // @ts-ignore
    Ethereum: {
      ...CONNECT_CONFIG.Devnet.chains.Ethereum,
      contracts: {
        tokenBridge: CONTRACTS.DEVNET.ethereum.token_bridge,
        coreBridge: CONTRACTS.DEVNET.ethereum.core,
      }
    }
  })
}
...
async function getSolTokenBridge(connection: Connection): Promise<TokenBridge<'Solana'>> {
  return SolanaTokenBridge.fromProvider(connection, {
    // @ts-ignore
    Solana: {
      ...CONNECT_CONFIG.Devnet.chains.Solana,
      contracts: {
        tokenBridge: CONTRACTS.DEVNET.solana.token_bridge,
        coreBridge: CONTRACTS.DEVNET.solana.core,
      }
    }
  })
