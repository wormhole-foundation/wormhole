...
1. sol_account (query type 10) - this query is used to read data for one or more accounts on Solana.
   ...
   - The `commitment` is required and must be either `confirmed` or `finalized`.
   ...
   - The `account_list` specifies a list of accounts to be batched into a single call. The `account` should be specified in base58 format.
