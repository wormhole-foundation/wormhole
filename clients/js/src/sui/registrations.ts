...
export async function query_registrations_sui(
  ...
  console.log(
    `Querying the ${module} on ${network} Sui for registered chains.`
  );
  ...
  for (let idx = 0; idx < emitterRegistry.data.length; idx++) {
    ...
    for (const { objectId } of emitterRegistry.data.slice(idx, idx + 1)) {
      ...
      const emitterAddrStr = "0x" + Buffer.from(emitterAddress).toString("hex");
