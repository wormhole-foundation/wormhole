...
exports.builder = (y: typeof yargs) => {
  ...
  .option("wormscanfile", {
    alias: "wsf",
    describe: "json file containing wormscan entry for the vaa that includes signatures",
    type: "string",
  })
...
exports.handler = async(argv) => {
  ...
  if (argv["wormscanfile"]) { numSigs += 1; }
  if (argv["wormscanurl"]) { numSigs += 1; }
  if (argv["guardian-secret"]) { numSigs += 1; }
  if (numSigs > 1) {
    throw new Error(`may only specify one of "--signatures", "--wormscanfile", "--wormscanurl" or "--guardian-secret"`);
  }
  ...
  if (argv["signatures"]) {
    ...
    const guardianSet = await getGuardianSet(argv["network"], vaa.guardianSetIndex);
    ...
    const guardianSet = await getGuardianSet(argv["network"], vaa.guardianSetIndex);
    vaa.signatures = await getSigsFromWormscanData(wormscanData.data, guardianSet);
...
async function getGuardianSet(nwork: string, guardianSetIndex: number): Promise<string[]> {
  if (! nwork) {
...
function getSigsFromWormscanData(wormscanData: any, guardianSet: string[]): any {
  let sigs: Signature[] = [];
  for (let data in wormscanData) {
    ...
    let sig: Signature = {
      ...
      signature: Buffer.from(wormscanData[data].signature, 'base64').toString('hex'),
    };
