...
function makeVAA(emitterChain: number, emitterAddress: string, signers: string[], p: Payload): VAA<Payload> {
  let v: VAA<Payload> = {
    version: 0,
    guardianSetIndex: 0,
    signatures: [],
...
yargs(hideBin(process.argv))
  ////////////////////////////////////////////////////////////////////////////////
  // Generate
  .command('generate', 'generate VAAs (devnet and testnet only)', (yargs) => {
    return yargs
      ...
      .command("registration", "Generate registration VAA", (yargs) => {
        ...
        let payload: vaa.PortalRegisterChain<typeof module> = {
          ...
          emitterAddress: Buffer.from(argv['contract-address'].padStart(64, "0"), "hex")
        }
        let v = makeVAA(GOVERNANCE_CHAIN, GOVERNANCE_EMITTER, argv['guardian-secret'], payload)
      ...
      .command("upgrade", "Generate contract upgrade VAA", (yargs) => {
        ...
        let payload: Payload = {
          ...
          address: Buffer.from(argv['contract-address'].padStart(64, "0"), "hex")
        }
        let v = makeVAA(GOVERNANCE_CHAIN, GOVERNANCE_EMITTER, argv['guardian-secret'], payload)
        console.log(serialiseVAA(v))
      })
  ...
  .command("submit <vaa>", "Execute a VAA", (yargs) => {
    ...
    if (cli_chain !== undefined) {
      ...
      if (vaa_chain !== "unset" && cli_chain !== vaa_chain) {
        throw Error(`Specified target chain (${cli_chain}) does not match VAA target chain (${vaa_chain})`)
      }
      chain = cli_chain
    } else {
      chain = vaa_chain
    ...
    if (chain === "unset") {
      throw Error("This VAA does not specify the target chain, please provide it by hand using the '--chain' flag.")
