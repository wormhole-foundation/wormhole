...
abstract contract VaaBuilder {
	...
	function newSchnorrShardData(ShardData[] memory shards) internal pure returns (bytes memory result) {
		assembly ("memory-safe") {
			let shardCount := mload(shards)
			let readPtr := add(shards, 0x20)

			result := mload(0x40)
			mstore(0x40, add(result, add(0x20, shl(6, shardCount))))

			let writePtr := add(result, 0x20)

			for { let i := 0 } lt(i, shardCount) { i := add(i, 1) } {
				mstore(writePtr, mload(readPtr))
				mstore(add(writePtr, 0x20), mload(add(readPtr, 0x20)))
	...
	function newMultisigVaa(uint32 guardianSetIndex, bytes memory signatures, bytes memory envelope) public pure returns (bytes memory) {
		uint8 guardianCount = uint8(signatures.length / 66);
...
contract TestAssembly2 is VerificationTestAPI {
	uint256 private constant EXPIRATION_DELAY_SECONDS = 24 * 60 * 60;
	...
	function setUp() public {
		...
		bytes memory schnorrShardsRaw = newSchnorrShardData(schnorrShards);
		...
		bytes memory appendSchnorrKeyMessage2 = newAppendSchnorrKeyMessage(1, pk2, 0, schnorrShardDataHash);
		...
		_wormholeVerifierV2.update(abi.encodePacked(
