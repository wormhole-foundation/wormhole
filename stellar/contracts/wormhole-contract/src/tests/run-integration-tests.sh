...
#   DEPLOYER_IDENTITY   Stellar identity name for deployment
#                       (default: the `deployer` field from
#                       scripts/config/<network>.yaml, i.e. whichever
#                       identity deploy.sh actually deploys with)
#
# TEST FLOW:
...
NETWORK="${1:-testnet}"
CONFIG_FILE="$STELLAR_ROOT/scripts/config/$NETWORK.yaml"
...
[[ -f "$CONFIG_FILE" ]] || die "missing deploy config: $CONFIG_FILE"
...
# Default to the same identity deploy.sh will actually deploy with (read from
# the same config file), so this never drifts out of sync with deploy.sh.
DEPLOYER_IDENTITY="${DEPLOYER_IDENTITY:-$(yq -r '.deployer' "$CONFIG_FILE")}"
