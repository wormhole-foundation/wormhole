...
Configuration:
  ...
  - guardians: Array of guardian addresses passed to the contract's
               __constructor (must be non-empty)
...
[[ "$GUARDIAN_COUNT" -gt 0 ]] || die "config has no guardians: $CONFIG_FILE"
...
# Deploy the contract.
#
# The contract initializes state via a `__constructor` that runs atomically at
# deploy time (Soroban Protocol 22+): the initial guardian set and governance
# emitter are passed here as constructor arguments. There is no separate
# `initialize` entrypoint.
step "deploy + init ($NETWORK, $GUARDIAN_COUNT guardians)"
DEPLOY_OUTPUT="$(stellar contract deploy --wasm "$CONTRACT_WASM" --source-account "$DEPLOYER" --network "$NETWORK" -- \
  --initial_guardians "$GUARDIANS_JSON" --governance_emitter "\"$GOVERNANCE_EMITTER\"" 2>&1)"
...
# Verify the constructor initialized state by checking guardian set index is 0
step "verify init"
...
printf '{"network":"%s","contract_id":"%s","wasm_hash":"%s","deployer":"%s","guardian_count":%s,"initialized":true,"timestamp":"%s"}\n' \
  "$NETWORK" "$CONTRACT_ID" "${WASM_HASH:-}" "$DEPLOYER_ADDR" "$GUARDIAN_COUNT" "$TIMESTAMP"
