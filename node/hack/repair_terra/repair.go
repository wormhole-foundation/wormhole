...
func getCurrentHeight() (uint64, error) {
	...
	terraWS := "http://localhost:1317"
	resp, err := client.Get(fmt.Sprintf("%s/blocks/latest", terraWS))
	if err != nil {
	...
	fmt.Println("current Terra height ", latestBlock.Int())
	return latestBlock.Uint(), nil
}

func getSequenceForTxhash(txhash string) (uint64, error) {
	// fmt.Println("Starting getSequence()...")
	client := &http.Client{ 
		Timeout: time.Second * 5, 
	}
...
func main() {
	flag.Parse()
	fmt.Println("terraAddr", *terraAddr)
	...
	for ((offset > 0) || firstTime) {
		...
		blockJSON := string(blocksBody)
		// fmt.Println("blockJSON", blockJSON)
