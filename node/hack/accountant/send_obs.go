...
func testSubmit(
	...
	if err != nil {
		logger.Error("gacct: failed to broadcast Observation request", zap.String("test", tag), zap.Error(err))
		return false
	}

	responses, err := accountant.GetObservationResponses(txResp)
	if err != nil {
		logger.Error("gacct: failed to get responses", zap.Error(err))
		return false
	}

	if len(responses) != len(msgs) {
		logger.Error("gacct: number of responses does not match number of messages", zap.Int("numMsgs", len(msgs)), zap.Int("numResp", len(responses)), zap.Error(err))
		return false
	}
...
func testBatch(
	...
	txResp, err := submit(ctx, logger, gk, wormchainConn, contract, msgs)
	if err != nil {
		logger.Error("gacct: failed to broadcast Observation request", zap.Error(err))
		return false
	}
	...
	for idx, msg := range msgs {
		...
		status, exists := responses[msgId]
		if !exists {
			logger.Error("gacct: did not receive an observation response for message", zap.Int("idx", idx), zap.String("msgId", msgId))
			return false
		}

		if status.Type != "committed" {
			logger.Error("gacct: unexpected response on observation", zap.Int("idx", idx), zap.String("msgId", msgId), zap.String("status", status.Type), zap.String("text", status.Data))
			return false
		}
	}
...
func testBatchWithcommitted(
	...
	if err != nil {
		logger.Error("gacct: failed to submit initial observation that should work", zap.Error(err))
		return false
	}
...
func testBatchWithDigestError(
	...
	txResp, err := submit(ctx, logger, gk, wormchainConn, contract, msgs)
	if err != nil {
		logger.Error("gacct: failed to submit second observation that should work", zap.Error(err))
		return false
	}
	...
	status, exists := responses[msgId]
	if !exists {
		logger.Error("gacct: did not receive an observation response for message 0", zap.String("msgId", msgId))
		return false
	}

	if status.Type != "committed" {
		logger.Error("gacct: unexpected response on observation for message 0", zap.String("msgId", msgId), zap.String("status", status.Type), zap.String("text", status.Data))
		return false
	}
	...
	status, exists = responses[msgId]
	if !exists {
		logger.Error("gacct: did not receive an observation response for message 1", zap.String("msgId", msgId))
		return false
	}

	if status.Type != "error" {
		logger.Error("gacct: unexpected response on observation for message 1", zap.String("status", status.Type), zap.String("text", status.Data))
		return false
	}

	if status.Data != "digest mismatch for processed message" {
		logger.Error("gacct: unexpected error text on observation for message 1", zap.String("text", status.Data))
		return false
	}
