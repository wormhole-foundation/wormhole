...
func runNode(cmd *cobra.Command, args []string) {
	...
	supervisor.New(rootCtx, logger, func(ctx context.Context) error {
		...
		if err := supervisor.Run(ctx, "p2p", p2p.Runnable(priv, gst, *p2pNetworkID, *p2pBootstrap, *nodeName, components, func(ctx context.Context, gossip *p2p.Gossip) supervisor.Runnable {
			return func(ctx context.Context) error {
				...
				if err := supervisor.Run(ctx, "heartbeat_sender", heartbeat.HeartbeatSenderRunnable(*nodeName, features, gk, heartbeatTopic, gossip.PeerID(), gst, gov, components)); err != nil {
					return err
				}
				if err := supervisor.Run(ctx, "heartbeat_processor", heartbeat.HeartbeatProcessorRunnable(gst, *disableHeartbeatVerify, heartbeatTopic, components)); err != nil {
					return err
				}
