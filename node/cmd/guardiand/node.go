	...
	supervisor.New(rootCtx, logger, func(ctx context.Context) error {
...

		{
			...
			if *polygonRootChainRpc != "" {
				if *polygonRootChainContractAddress == "" {
					log.Fatal("if \"polygonRootChainRpc\" is specified then \"polygonRootChainContractAddress\" is required", zap.Error(err))
				}
				extraParams = []string{*polygonRootChainRpc, *polygonRootChainContractAddress}
				minConfirmations = 1
			} else if *testnetMode {
				minConfirmations = 64
			}
		...
		if *testnetMode {
			...
			if err := supervisor.Run(ctx, "neonwatch",
				ethereum.NewEthWatcher(*neonRPC, neonContractAddr, "neon", common.ReadinessNeonSyncing, vaa.ChainIDNeon, lockC, nil, 32, chainObsvReqC[vaa.ChainIDNeon], *unsafeDevMode, nil).Run); err != nil {
				return err
			}
		}
