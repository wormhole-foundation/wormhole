	...
	go func() {
		for {
			...
			default:
				...
				if err != nil {
					s.errC <- fmt.Errorf("failed to subscribe to logs: %w", err) // can_block: Will only block this subscriber routine
				...
				if err != nil {
					s.errC <- err               // can_block: Will only block this subscriber routine
					time.Sleep(RECONNECT_DELAY) // Wait before retrying
				}
			}
		}
