...
import (
	...
	celo_common "github.com/celo-org/celo-blockchain/common"
	...
	"github.com/certusone/wormhole/node/pkg/celo/abi"
...
var (
	ethConnectionErrors = promauto.NewCounterVec(
		prometheus.CounterOpts{
			Name: "wormhole_celo_connection_errors_total",
			Help: "Total number of celo connection errors (either during initial connection or while watching)",
		}, []string{"celo_network", "reason"})

	ethMessagesObserved = promauto.NewCounterVec(
		prometheus.CounterOpts{
			Name: "wormhole_celo_messages_observed_total",
			Help: "Total number of Eth messages observed (pre-confirmation)",
		}, []string{"celo_network"})
	ethMessagesOrphaned = promauto.NewCounterVec(
		prometheus.CounterOpts{
			Name: "wormhole_celo_messages_orphaned_total",
			Help: "Total number of Eth messages dropped (orphaned)",
		}, []string{"celo_network", "reason"})
	ethMessagesConfirmed = promauto.NewCounterVec(
		prometheus.CounterOpts{
			Name: "wormhole_celo_messages_confirmed_total",
			Help: "Total number of Eth messages verified (post-confirmation)",
		}, []string{"celo_network"})
	currentEthHeight = promauto.NewGaugeVec(
		prometheus.GaugeOpts{
			Name: "wormhole_celo_current_height",
			Help: "Current celo block height",
		}, []string{"celo_network"})
	queryLatency = promauto.NewHistogramVec(
		prometheus.HistogramOpts{
			Name: "wormhole_celo_query_latency",
			Help: "Latency histogram for celo calls (note that most interactions are streaming queries, NOT calls, and we cannot measure latency for those",
		}, []string{"celo_network", "operation"})
)
...
type (
	Watcher struct {
		...
		//
		// The current primary chain is celo (a mostly arbitrary decision because it
	...
	pendingKey struct {
		TxHash         celo_common.Hash
		BlockHash      celo_common.Hash
...
func NewCeloWatcher(
	...
	return &Watcher{
		url:              url,
		contract:         celo_common.BytesToAddress(contract),
...
func (e *Watcher) Run(ctx context.Context) error {
	...
	if err != nil {
		...
		return fmt.Errorf("dialing celo client failed: %w", err)
	}
	...
	go func() {
		for {
			...
			case r := <-e.obsvReqC:
				...
				tx := celo_common.BytesToHash(r.TxHash)
				logger.Info("received observation request",
					zap.String("celo_network", e.networkName),
				...
				if err != nil {
					logger.Error("failed to process observation request",
						zap.Error(err), zap.String("celo_network", e.networkName))
					continue
				}
	...
	go func() {
		for {
			...
			case ev := <-messageC:
				...
				message := &common.MessagePublication{
					TxHash:           eth_common.Hash(ev.Raw.TxHash),
				...
				key := pendingKey{
					TxHash:         celo_common.BytesToHash(message.TxHash.Bytes()),
			...
			case ev := <-headSink:
				...
				for key, pLock := range e.pending {
					...
					if pLock.height+uint64(expectedConfirmations) <= blockNumberU {
						timeout, cancel := context.WithTimeout(ctx, 5*time.Second)
						tx, err := c.TransactionReceipt(timeout, celo_common.BytesToHash(pLock.message.TxHash.Bytes()))
						cancel()
...
func convertKeys(keys []celo_common.Address) []eth_common.Address {
	r := make([]eth_common.Address, len(keys))

	for n, k := range keys {
		r[n] = eth_common.BytesToAddress(k.Bytes())
	}
