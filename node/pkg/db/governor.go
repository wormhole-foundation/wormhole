...
func (t *Transfer) Marshal() ([]byte, error) {
	...
	vaa.MustWrite(buf, binary.BigEndian, t.ScaledValue)
...
func UnmarshalTransfer(data []byte) (*Transfer, error) {
	...
	if err := binary.Read(reader, binary.BigEndian, &t.ScaledValue); err != nil {
...
const (
	...
	// Transfers.
	oldTransferPrefix = "GOV:XFER4:"
	oldTransferLen    = len(oldTransferPrefix)
	transferPrefix    = "GOV:XFER5:"
	...
	err = d.db.View(func(txn *badger.Txn) error {
		...
		for it.Rewind(); it.Valid(); it.Next() {
			...
			if IsPendingMsg(key) {
				...
				// Old transfers (XFER4) store unscaled USD values.
				// We need to scale them up by ScaledValueFactor for the new format (XFER5).
				v, err := UnmarshalTransfer(val)
				if err != nil {
					return err
				}

				// Scale up the value to match the new format
				// The variable name is a bit misleading here as it is unscaled in the old format
				// but we marshall it into the renamed field.
				v.ScaledValue = v.ScaledValue * ScaledValueFactor
		...
		if len(oldTransfers) != 0 {
			for _, xfer := range oldTransfers {
				logger.Info("updating format of database entry for completed transfer (scaling value)", zap.String("msgId", xfer.MsgID), zap.Uint64("scaledValue", xfer.ScaledValue))
