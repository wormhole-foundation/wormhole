...
func TestSerializeAndDeserializeOfTransfer(t *testing.T) {
	...
	xfer2, err := db.UnmarshalTransfer(bytes)
	assert.Nil(t, err)
	...
	assert.Equal(t, expectedTransferKey, string(db.TransferMsgID(xfer2)))
}
...
func TestSerializeAndDeserializeOfMessagePublication(t *testing.T) {
	...
	msg2, err := common.UnmarshalMessagePublication(bytes)
	assert.Nil(t, err)
	...
	assert.Equal(t, expectedPendingKey, string(db.PendingMsgID(msg2)))
}
