...
//
var KnownEmitters = initializeKnownEmitters()

func initializeKnownEmitters() []EmitterInfo {
	out := make([]EmitterInfo, 0, len(knownTokenbridgeEmitters)+len(knownNFTBridgeEmitters))
	for id, emitter := range knownTokenbridgeEmitters {
	...
	for id, emitter := range knownNFTBridgeEmitters {
		out = append(out, EmitterInfo{
			ChainID:    id,
			Emitter:    emitter,
