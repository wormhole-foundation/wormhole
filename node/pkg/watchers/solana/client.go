		...
		for {
			...
			default:
				if msg, err := s.readWebSocketWithTimeout(ctx, ws); err != nil {
					...
					s.pumpData <- msg // can_block: Only pauses this watcher
				}
			}
		}
	})
		...
		for {
			...
			case msg := <-s.pumpData:
				...
				if err != nil {
					...
					s.errC <- err // can_block: The watcher will exit anyway
					return err
				}
			case m := <-s.obsvReqC:
...
func (s *SolanaWatcher) processMessageAccount(logger *zap.Logger, data []byte, acc solana.PublicKey, isReobservation bool) (numObservations uint32) {
	...
	s.msgC <- observation //can_block: The channel to the processor is buffered and shared across chains, if it backs up we should stop processing new observations
	return 1
}
