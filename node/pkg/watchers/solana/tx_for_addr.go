...
func (s *SolanaWatcher) getPrevWormholeSignature() (solana.Signature, error) {
	...
	if len(signatures) == 0 {
		s.logger.Info("start up query did not return any transaction")
...
func (s *SolanaWatcher) getTransactionSignatures() ([]*rpc.TransactionSignature, error) {
	...
	for numSignatures == MaxSignaturesPerQuery {
		if time.Since(startTime) > time.Minute {
			...
			// attempt to process the transactions because we read them from most recent backwards, so there would
			// be a gap and we would miss old transactions.
