...
// deserializes. No Address Lookup Tables are ever emitted; every transaction is
// legacy, so account indices equal the static account-key list verbatim.
package testgen
...
// set to the message account index (the slot the watcher reads).
func accountRefs(minLen int, msgIdx uint16) []uint16 {
	if minLen < 2 {
		minLen = 2
	}
