...
func (s *SolanaWatcher) shimProcessTopLevelInstruction(
	...
	// The only top-level instruction generated by the shim contract is the PostMessage event.
	// The deterministic builder parses the full payload once the matching inner event is found.
	...
	err := s.shimProcessRest(logger, whProgramIndex, shimProgramIndex, tx, innerInstructions[outerIdx].Instructions, outerIdx, 0, inst.Data, alreadyProcessed, isReobservation, true)
	if err != nil {
...
func (s *SolanaWatcher) shimProcessInnerInstruction(
	...
	// See if this is a PostMessage event from the shim contract. The deterministic builder parses the full payload once the matching inner event is found.
	if !shimMatchPrefix(s.shimPostMessageDiscriminator, innerInstructions[startIdx].Data) {
		return false, nil
	}

	alreadyProcessed.add(outerIdx, startIdx)

	err := s.shimProcessRest(logger, whProgramIndex, shimProgramIndex, tx, innerInstructions, outerIdx, startIdx+1, innerInstructions[startIdx].Data, alreadyProcessed, isReobservation, false)
	if err != nil {
...
func (s *SolanaWatcher) shimProcessRest(
	...
	outerIdx int,
	startIdx int,
	postMessageInstructionData []byte,
	alreadyProcessed ShimAlreadyProcessed,
	isReobservation bool,
	isTopLevel bool,
) error {
	if postMessageInstructionData == nil {
		return errors.New("postMessageInstructionData is nil")
	}
	...
	var messageEventInstructionData []byte
	for idx := startIdx; idx < len(innerInstructions); idx++ {
		...
		if inst.ProgramIDIndex == whProgramIndex {
			...
			// SECURITY: Validate the event type. The deterministic builder validates the event data once the full shim publication inputs are available.
			...
			if shimMatchPrefix(s.shimMessageEventDiscriminator, inst.Data) {
				if !verifiedCoreEvent {
					return fmt.Errorf("detected an inner shim message event instruction before the core event for shim instruction %d, %d", outerIdx, idx)
				}

				if messageEventInstructionData != nil {
				...
				messageEventInstructionData = inst.Data
				alreadyProcessed.add(outerIdx, idx)
			}
		}

		if verifiedCoreEvent && messageEventInstructionData != nil {
	...
	observation, err := s.buildMessagePublicationFromShimData(tx.Signatures[0], postMessageInstructionData, messageEventInstructionData, isReobservation)
	if err != nil {
		return fmt.Errorf("failed to build shim message publication for shim instruction %d, %d: %w", outerIdx, startIdx, err)
	}

	commitment, err := ConsistencyLevel(observation.ConsistencyLevel).Commitment()
	if err != nil {
