...
<!-- cspell:ignore XTCF XACK -->

### Generated Message Emitter Address

The XTCF and XACK messages described below are generated by the watcher itself in response to transactions on a managed account, rather than being requested by an integrator. A generic [Core message](#generic-messages) uses the raw payment sender account as its emitter — the 20-byte account ID left-padded with 12 zero bytes. To make it impossible to forge an XTCF or XACK by sending a Payment to the Core Account, these generated messages instead use an emitter address that replaces those leading zero bytes with a non-zero `"XRPL"` prefix:

| Offset | Size | Field                        |
| ------ | ---- | ---------------------------- |
| 0      | 4    | prefix ("XRPL" = 0x5852504C) |
| 4      | 8    | zero padding                 |
| 12     | 20   | XRPL account id              |

Total: 32 bytes

Because the leading 12 bytes are never all-zero, this emitter can never collide with a Core message emitter. The account ID remains in the last 20 bytes, so it can still be recovered from the emitter. Both XTCF and XACK messages from a given account share this emitter and are distinguished by their payload prefix.
...
The XRPL account is identified by the VAA emitter (see [Generated Message Emitter Address](#generated-message-emitter-address)), not a payload field.
