...
func (b *BatchPollConnector) SubscribeForBlocks(ctx context.Context, errC chan error, sink chan<- *NewBlock) (ethereum.Subscription, error) {
	...
	for idx, block := range lastBlocks {
		...
		sink <- block //can_block: This channel is buffered, if it backs up, we will just stop polling until it clears
		if b.generateSafe && b.batchData[idx].finality == Finalized {
			...
			sink <- safe //can_block: This channel is buffered, if it backs up, we will just stop polling until it clears
		}
	}
		...
		for {
			...
			case <-timer.C:
				...
				if err != nil {
					...
					if errCount > 3 {
						errC <- fmt.Errorf("polling encountered too many errors: %w", err) // can_block: The watcher will exit anyway
						return nil
					}
				} else if errCount != 0 {
					errCount = 0
			...
			case ev := <-headSink:
				...
				sink <- &NewBlock{ //can_block: This channel is buffered, if it backs up, we will just stop polling until it clears
					Number:   ev.Number,
					Time:     ev.Time,
					Hash:     ev.Hash(),
					Finality: Latest,
	...
	for idx, newBlock := range newBlocks {
		if newBlock.Number.Cmp(prevBlocks[idx].Number) > 0 {
			...
			for blockNum < newBlockNum && !errorFound {
				...
				if err != nil {
					...
					for _, block := range gapBlocks {
						...
						sink <- block //can_block: This channel is buffered, if it backs up, we will just stop polling until it clears
						if b.generateSafe && b.batchData[idx].finality == Finalized {
							sink <- block.Copy(Safe) //can_block: This channel is buffered, if it backs up, we will just stop polling until it clears
						}
			...
			if !errorFound {
				...
				sink <- newBlock //can_block: This channel is buffered, if it backs up, we will just stop polling until it clears
				if b.generateSafe && b.batchData[idx].finality == Finalized {
					sink <- newBlock.Copy(Safe) //can_block: This channel is buffered, if it backs up, we will just stop polling until it clears
				}
			} else {
