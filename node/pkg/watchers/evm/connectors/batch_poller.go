...
func (b *BatchPollConnector) SubscribeForBlocks(ctx context.Context, errC chan error, sink chan<- *NewBlock) (ethereum.Subscription, error) {
	...
	for idx, block := range lastBlocks {
		...
		sink <- block // Note on channel capacity: This channel is buffered, if it backs up, we will just stop polling until it clears
		if b.generateSafe && b.batchData[idx].finality == Finalized {
			...
			sink <- safe // Note on channel capacity: This channel is buffered, if it backs up, we will just stop polling until it clears
		}
	}
	...
	common.RunWithScissors(ctx, errC, "block_poll_subscribe_for_blocks", func(ctx context.Context) error {
		...
		for {
			...
			case ev := <-headSink:
				...
				sink <- &NewBlock{ // Note on channel capacity: This channel is buffered, if it backs up, we will just stop polling until it clears
					Number:   ev.Number,
					Time:     ev.Time,
					Hash:     ev.Hash(),
					Finality: Latest,
	...
	for idx, newBlock := range newBlocks {
		if newBlock.Number.Cmp(prevBlocks[idx].Number) > 0 {
			...
			for blockNum < newBlockNum && !errorFound {
				...
				if err != nil {
					...
					for _, block := range gapBlocks {
						...
						sink <- block // Note on channel capacity: This channel is buffered, if it backs up, we will just stop polling until it clears
						if b.generateSafe && b.batchData[idx].finality == Finalized {
							sink <- block.Copy(Safe) // Note on channel capacity: This channel is buffered, if it backs up, we will just stop polling until it clears
						}
			...
			if !errorFound {
				...
				sink <- newBlock // Note on channel capacity: This channel is buffered, if it backs up, we will just stop polling until it clears
				if b.generateSafe && b.batchData[idx].finality == Finalized {
					sink <- newBlock.Copy(Safe) // Note on channel capacity: This channel is buffered, if it backs up, we will just stop polling until it clears
				}
			} else {
