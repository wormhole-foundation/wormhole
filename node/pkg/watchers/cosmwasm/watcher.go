		...
		for {
			...
			case <-t.C:
				...
				if err != nil {
					...
					errC <- err // can_block: The watcher will exit anyway
					resp.Body.Close()
					continue
				}
				resp.Body.Close()
		...
		for {
			...
			case r := <-e.obsvReqC:
				...
				for _, msg := range msgs {
					...
					e.msgC <- msg //can_block: The channel to the processor is buffered and shared across chains, if it backs up we should stop processing new observations
