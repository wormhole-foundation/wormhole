...
func (w *Watcher) Run(ctx context.Context) error {
	...
	w.logger.Info("Starting watcher",
		...
		zap.String("suiRPC", w.suiRPC),
		zap.String("suiWS", w.suiWS),
		zap.String("suiMoveEventType", w.suiMoveEventType),
...
func (w *Watcher) queryBlockHeight(ctx context.Context) error {
	...
	for {
		...
		case <-timer.C:
			resp, err := http.Post(w.suiRPC, "application/json", strings.NewReader(`{"jsonrpc":"2.0", "id": 1, "method": "sui_getLatestCheckpointSequenceNumber", "params": []}`)) //nolint:noctx
			if err != nil {
...
func (w *Watcher) handleObservationRequests(ctx context.Context) error {
	for {
		...
		case r := <-w.obsvReqC:
			...
			resp, err := http.Post(w.suiRPC, "application/json", strings.NewReader(buf)) //nolint:noctx
			if err != nil {
