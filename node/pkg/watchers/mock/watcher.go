	...
	return func(ctx context.Context) error {
		...
		for {
			...
			case observation := <-c.MockObservationC:
				...
				msgC <- observation // Note on channel capacity: Will only block this mock watcher. Shouldn't drop observations anyway.
			case gs := <-c.MockSetC:
				setC <- gs //nolint:channelcheck // Will only block this mock watcher. Shouldn't drop guardian set updates anyway.
			case o := <-obsvReqC:
				...
				if ok {
					...
					msgC <- &msg2 // Note on channel capacity: Will only block this mock watcher. Shouldn't drop observations anyway.
				}
			}
		}
	}
