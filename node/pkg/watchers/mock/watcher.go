	...
	return func(ctx context.Context) error {
		...
		for {
			...
			case observation := <-c.MockObservationC:
				...
				msgC <- observation //can_block: The channel to the processor is buffered and shared across chains, if it backs up we should stop processing new observations
			case gs := <-c.MockSetC:
				setC <- gs //can_block: Will only block this mock watcher
			case o := <-obsvReqC:
				...
				if ok {
					...
					msgC <- &msg2 //can_block: The channel to the processor is buffered and shared across chains, if it backs up we should stop processing new observations
				}
			}
		}
	}
