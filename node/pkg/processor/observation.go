...
func (p *Processor) handleObservation(ctx context.Context, m *gossipv1.SignedObservation) {
	...
	if p.state.signatures[hash].ourObservation != nil {
		...
		if len(sigs) >= quorum && !p.state.signatures[hash].submitted {
			switch o := p.state.signatures[hash].ourObservation.(type) {
			case *vaa.VAA:
				p.handleVAAQuorum(o, sigs, hash)
			default:
				panic("unknown observation type")
			}
		} else {
...
func (p *Processor) handleVAAQuorum(v *vaa.VAA, sigs []*vaa.Signature, hash string) {
