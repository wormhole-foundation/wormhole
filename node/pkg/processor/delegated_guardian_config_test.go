...
func TestNewDelegatedGuardianChainConfig(t *testing.T) {
	keys := []common.Address{
	...
	threshold := vaa.CalculateQuorum(len(keys))
	cfg, err := NewDelegatedGuardianChainConfig(keys, threshold)
	assert.NoError(t, err)
	assert.True(t, reflect.DeepEqual(keys, cfg.Keys))
	assert.Equal(t, threshold, cfg.Quorum())
}
...
func TestThreshold(t *testing.T) {
	...
	maxThreshold := len(keys)
	minThreshold := vaa.CalculateQuorum(maxThreshold)

	// Threshold too low
	cfg, err := NewDelegatedGuardianChainConfig(keys, minThreshold-1)
	assert.Error(t, err)
	assert.Nil(t, cfg)

	// Threshold lower limit
	cfg, err = NewDelegatedGuardianChainConfig(keys, minThreshold)
	assert.NoError(t, err)
	assert.True(t, reflect.DeepEqual(keys, cfg.Keys))
	assert.Equal(t, minThreshold, cfg.Quorum())

	// Arbitrary threshold within limits
	threshold := (minThreshold/2 + maxThreshold/2)
	cfg, err = NewDelegatedGuardianChainConfig(keys, threshold)
	assert.NoError(t, err)
	assert.True(t, reflect.DeepEqual(keys, cfg.Keys))
	assert.Equal(t, threshold, cfg.Quorum())
	...
	cfg, err = NewDelegatedGuardianChainConfig(keys, maxThreshold)
	assert.NoError(t, err)
	assert.True(t, reflect.DeepEqual(keys, cfg.Keys))
	assert.Equal(t, maxThreshold, cfg.Quorum())

	// Threshold too high
	cfg, err = NewDelegatedGuardianChainConfig(keys, maxThreshold+1)
	assert.Error(t, err)
	assert.Nil(t, cfg)
...
func TestNoDuplicateKeys(t *testing.T) {
	...
	cfg, err := NewDelegatedGuardianChainConfig(keys, 2)
	assert.Error(t, err)
	assert.Nil(t, cfg)
}
...
func TestKeysAsHexStrings(t *testing.T) {
	...
	keyStrings := make([]string, len(cfg.Keys))
	...
	assert.Equal(t, keyStrings, cfg.KeysAsHexStrings())
}
...
func TestKeyIndex(t *testing.T) {
	...
	tests := []test{
		{cfg: *cfg, address: "0x5aaeb6053f3e94c9b9a09f33669435e7ef1beaed", result: true, keyIndex: 0},
		{cfg: *cfg, address: "0x5aaeb6053f3e94c9b9a09f33669435e7ef1beaee", result: true, keyIndex: 1},
		{cfg: *cfg, address: "0x5aaeb6053f3e94c9b9a09f33669435e7ef1beaef", result: false, keyIndex: -1},
	}

	for _, testCase := range tests {
		t.Run(testCase.address, func(t *testing.T) {
			dgc := testCase.cfg
			keyIndex, result := dgc.KeyIndex(common.HexToAddress(testCase.address))
...
func TestSet(t *testing.T) {
	keys := []common.Address{
		...
		common.HexToAddress("0x5aaeb6053f3e94c9b9a09f33669435e7ef1beaed"),
		common.HexToAddress("0x5aaeb6053f3e94c9b9a09f33669435e7ef1beaee"),
		common.HexToAddress("0x5aaeb6053f3e94c9b9a09f33669435e7ef1beaef"),
	}
	cfg1, err := NewDelegatedGuardianChainConfig(keys[:2], 2)
	assert.NoError(t, err)
...
func TestGetChainConfig(t *testing.T) {
	...
	tests := []test{
		{chain: vaa.ChainIDArbitrumSepolia, keys: keys[:2], threshold: 2, exists: true},
		{chain: vaa.ChainIDBaseSepolia, keys: keys, threshold: 3, exists: true},
		{chain: vaa.ChainIDFogo, exists: false},
	}
