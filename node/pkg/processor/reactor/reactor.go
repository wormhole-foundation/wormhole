...
func NewReactor[K Observation](group string, config *Config, gs *node_common.GuardianSet, consensusSender p2p.GossipSender, s chan *StateTransition[K]) *ConsensusReactor[K] {
...
func (c *ConsensusReactor[K]) Run(ctx context.Context) error {
	...
	for {
		...
		case o := <-c.foreignObservationChannel:
			c.foreignObservationReceived(ctx, o)
		case <-ticker.C:
...
func (c *ConsensusReactor[K]) foreignObservationReceived(ctx context.Context, m *gossipv1.SignedObservation) {
	c.state.mutex.Lock()
	defer c.state.mutex.Unlock()
