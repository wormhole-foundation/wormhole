...
// GossipCutoverComplete returns true if the cutover time has not yet passed, meaning we should still publish on the old topic.
...
// GossipAttestationMsg is the payload of the `gossipAttestationSendC` channel. This will be used in stead of just `[]byte`
// until after the cutover is complete and support for publishing `SignedObservations` is removed.
