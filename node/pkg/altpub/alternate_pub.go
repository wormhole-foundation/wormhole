...
// TODO: Any other headers needed on our HTTP POST requests?
// TODO: Is there anything other than status 200 that should be treated as success?
// TODO: Do I need to do anything special to support https?
// TODO: Think about transport tuning parameters.
// Question: does Wormholescan handle Pyth traffic? If not, does it make sense to support "Give me everything *except* these chains"?
...
type (
	...
	AlternatePublisher struct {
		// logger ues the "altpub" component to identify our log messages.
		logger *zap.Logger

		// guardianAddr is used in gossipv1.Observation.
		guardianAddr []byte
	...
	// EnabledChains extends map to implement `String`.
	EnabledChains map[vaa.ChainID]struct{}

	// HttpRequest is the object sent to the worker for publishing.
	HttpRequest struct {
		// start is used for computing wormhole_alt_pub_channel_delay_in_ms.
		start time.Time
...
var (
	...
	channelDelay = promauto.NewHistogramVec(
		prometheus.HistogramOpts{
			Name:    "wormhole_alt_pub_channel_delay_in_ms",
			Help:    "Latency histogram for the time it took the request to reach the publisher in milliseconds",
...
func (e EnabledChains) String() string {
	...
	str := ""
	chainIds := slices.Sorted(maps.Keys(e))
...
func parseEndpoint(config string) (*Endpoint, error) {
	...
	enabledChains := make(map[vaa.ChainID]struct{})
	if len(fields) > 3 {
		chainIds := strings.Split(fields[3], ",")
	...
	// Format our topic URL and add the endpoint to the list.
	ep.createUrls()
	return ep, nil
}
...
func (ap *AlternatePublisher) httpPost(ctx context.Context, client *http.Client, req *HttpRequest) {
	channelDelay.WithLabelValues(req.ep.label).Observe(float64(time.Since(req.start).Milliseconds()))

	// Create the HTTP POST request using our context so it can be interrupted. Note that we are not using a timeout context
	// because the HTTP client already has a timeout.
...
func (ep *Endpoint) shouldPublish(chainId vaa.ChainID) bool {
	if len(ep.enabledChains) == 0 {
		return true
	}

	_, ok := ep.enabledChains[chainId]
	return ok
}
