...
func TestEnabledChainsString(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(fmt.Sprintf("%d", tc.input), func(t *testing.T) {
			...
			require.Equal(t, tc.output, EnabledChains(enabledChains).String())
		})
	}
}
	...
	for _, tc := range tests {
		t.Run(tc.label, func(t *testing.T) {
			...
			if tc.errText == "" {
				...
				require.True(t, slices.Equal(tc.chains, slices.Sorted(maps.Keys(ep.enabledChains))))
...
func TestShouldPublish(t *testing.T) {
	ep := &Endpoint{
		...
		baseUrl: "some_url",
		delay:   Delay(0),
		enabledChains: EnabledChains(map[vaa.ChainID]struct{}{
			vaa.ChainIDSolana: {},
		}),
	}
