...
const SPECIFIC_CORE_BRIDGE_TYPES = ["ChainId", "ExternalAddress"];
...
async function main() {
  ...
  // First copy core bridge types.
  const idl = fs.readFileSync(`${IDL_ROOT}/${CORE_BRIDGE_IDL}`, "utf8");

  fs.writeFileSync(`${IDL_OUT}/${CORE_BRIDGE_IDL}`, idl);
  fs.writeFileSync(
    `${TYPES_OUT}/${CORE_BRIDGE_IDL.replace(".json", ".ts")}`,
    fs.readFileSync(
      `${TYPES_ROOT}/${CORE_BRIDGE_IDL.replace(".json", ".ts")}`,
      "utf8"
    )
  );
