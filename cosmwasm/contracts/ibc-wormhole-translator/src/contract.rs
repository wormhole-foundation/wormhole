...
fn submit_vaa(
    ...
    match payload {
        ...
        Either::Right(message) => match message.action {
            ...
            //     &message.payload,
            // ),
            _ => ContractError::InvalidVAAAction1.std_err(),
        },

        #[cfg(not(feature = "full"))]
        _ => ContractError::InvalidVAAAction2.std_err(),
    }
}
...
fn handle_governance_payload(
    ...
    match gov_packet.action {
        ...
        _ => ContractError::InvalidVAAAction3.std_err(),
    }
}
...
fn query_transfer_info(deps: Deps, env: Env, vaa: &Binary) -> StdResult<TransferInfoResponse> {
    ...
    if is_governance_emitter(&cfg, parsed.emitter_chain, &parsed.emitter_address) {
        return ContractError::InvalidVAAAction4.std_err();
    }

    let message = TokenBridgeMessage::deserialize(&data)?;
    match message.action {
        Action::ATTEST_META => ContractError::InvalidVAAAction5.std_err(),
        Action::TRANSFER => {
