...
use cosmwasm_std::{testing::{mock_dependencies, mock_info, mock_env}, Binary, IbcChannel, IbcEndpoint, IbcOrder};
use wormhole::msg::{ExecuteMsg, InstantiateMsg};

use super::{execute, instantiate, find_wormchain_channel_id, WORMCHAIN_IBC_RECEIVER_PORT};
...
fn execute_post_message_ibc_happy_path() {
    ...
    let inst_res = instantiate(deps.as_mut(), mock_env(), inst_info, InstantiateMsg { 
        gov_chain: 1,
        gov_address: Binary::from_base64("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAQ=").unwrap(),
    ...
    let execute_info = mock_info("terra14hj2tavq8fpesdwxxcu44rty3hh90vhujrvcmstl4zr3txmfvw9ssrc8au", &[]);
    let wh_message = Binary::from_base64("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAQ=").unwrap();
    let res = execute(deps.as_mut(), mock_env(), execute_info, ExecuteMsg::PostMessage { message: wh_message, nonce: 1 }).unwrap();
...
fn find_wormchain_channel_id_happy_path() {
    ...
    let channel = IbcChannel::new(
        IbcEndpoint { port_id: String::from("transfer"), channel_id: String::from("channel-0") },
        IbcEndpoint { port_id: String::from(WORMCHAIN_IBC_RECEIVER_PORT), channel_id: String::from(expected_channel_id) },
        IbcOrder::Unordered,
        String::from("version"),
        String::from("connection-0")
    );
