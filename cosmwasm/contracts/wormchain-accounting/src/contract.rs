...
fn handle_observation(
    ...
    let registered_emitter = CHAIN_REGISTRATIONS
        ...
        registered_emitter.0 == **o.key.emitter_address(),
...
fn handle_governance_vaa(
    ...
    let key = GOVERNANCE_VAAS.key(digest.hash.to_vec());
    ...
    let evt = match body.payload.action {
        Action::RegisterChain {
            ...
            CHAIN_REGISTRATIONS
                .save(deps.storage, chain.into(), &emitter_address)
...
fn query_chain_registration(
    ...
    CHAIN_REGISTRATIONS
        ...
        .map(|addr| ChainRegistrationResponse {
            address: addr.0.to_vec().into(),
        })
}
