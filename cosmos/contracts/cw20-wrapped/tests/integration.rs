static WASM: &[u8] = include_bytes!("../../../target/wasm32-unknown-unknown/release/cw20_wrapped.wasm");

use cw20_wrapped::msg::{InitMsg, QueryMsg, HandleMsg};
use cw20_wrapped::state::{WrappedAssetInfo};
use cw20_wrapped::ContractError;
use cw20_wrapped::state::{KEY_WRAPPED_ASSET};
use cosmwasm_std::{from_slice, HumanAddr, Env, InitResponse, HandleResponse, Uint128, HandleResult};
use cosmwasm_storage::to_length_prefixed;
use cosmwasm_vm::testing::{mock_instance, mock_env, init, query, handle, MockStorage, MockApi, MockQuerier};
use cosmwasm_vm::{Storage, Api, Instance};
...
impl TestAddress {
    fn value(&self) -> HumanAddr {
        match self {
            ...
            TestAddress::ASSET => HumanAddr::from("addr1111"),
...
fn do_init(height: u64) -> Instance<MockStorage, MockApi, MockQuerier> {
    ...
    let init_msg = InitMsg {
        asset_chain: 1,
        asset_address: TestAddress::ASSET.value(),
        decimals: 10,
    };
    ...
    deps.with_storage(|storage| {
        assert_eq!(
            ...
            WrappedAssetInfo {
                asset_chain: 1,
                asset_address: api.canonical_address(&TestAddress::ASSET.value()).0?,
...
fn do_mint(deps: &mut Instance<MockStorage, MockApi, MockQuerier>, height: u64, recipient: &HumanAddr, amount: &Uint128) {
    let mint_msg = HandleMsg::Mint {
        recipient: recipient.clone(), amount: amount.clone()
    };
    let env = mock_env_height(&TestAddress::INITIALIZER.value(), height, 0);
...
fn do_transfer(deps: &mut Instance<MockStorage, MockApi, MockQuerier>, height: u64, sender: &HumanAddr, recipient: &HumanAddr, amount: &Uint128) {
...
fn check_balance(deps: &mut Instance<MockStorage, MockApi, MockQuerier>, address: &HumanAddr, amount: &Uint128) {
    let query_response = query(deps, QueryMsg::Balance { address: address.clone()}).unwrap();
    assert_eq!(query_response.as_slice(), format!(
        "{{\"balance\":\"{}\"}}", amount.u128()).as_bytes());
}
...
fn check_token_details(deps: &mut Instance<MockStorage, MockApi, MockQuerier>, supply: &Uint128) {
    ...
    assert_eq!(query_response.as_slice(), format!(
        ...
        \"symbol\":\"WWT\",\
        \"decimals\":10,\
        \"total_supply\":\"{}\"}}", supply.u128()).as_bytes());
}
...
fn query_works() {
    ...
    assert_eq!(query_response.as_slice(), format!(
        ...
        \"asset_address\":\"{}\",\
        \"bridge\":\"{}\"}}", 
        TestAddress::ASSET.value().as_str(), 
        TestAddress::INITIALIZER.value().as_str()).as_bytes());
}
...
fn mint_works() {
    ...
    do_mint(&mut deps, 112, &TestAddress::RECIPIENT.value(), &Uint128(123_123_123));
...
fn others_cannot_mint() {
    ...
    assert_eq!(format!("{}", handle_result.unwrap_err()), format!("{}", ContractError::Unauthorized {}));
}
...
fn transfer_works() {
    ...
    do_mint(&mut deps, 112, &TestAddress::SENDER.value(), &Uint128(123_123_123));
    ...
    check_balance(&mut deps, &TestAddress::RECIPIENT.value(), &Uint128(123_123_000));
}
...
fn cannot_burn_more_than_available() {
    ...
    let env = mock_env_height(&TestAddress::INITIALIZER.value(), 113, 0);
    let handle_result: HandleResult<HandleResponse> = handle(&mut deps, env, burn_msg);
    assert_eq!(format!("{}", handle_result.unwrap_err()), "Cannot subtract 123123124 from 123123123");
}