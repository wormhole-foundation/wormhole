...
function execWrapper(command) {
    execSync(command, (error, stdout, stderr) => {
        if (error) {
            console.log(`error while processing command - ${command}: ${error.message}`);
            return;
        }
        if (stderr) {
...
certusFiles.forEach((directory) => {
    ...
    execWrapper(`cp -R ${CERTUS_DIRECTORY + directory.name}/module/* ${MODULE_DIRECTORY + directory.name}/`); //move all the files from the vue module into the sdk
});
